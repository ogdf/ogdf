...
public:
	...
	void removeReinsertFix(RemoveReinsertType rrOption) { m_rrOptionFix = rrOption; }

	//! Returns the current setting of the remove-reinsert postprocessing method.
	RemoveReinsertType removeReinsertFix() const { return m_rrOptionFix; }

	//! Sets the remove-reinsert postprocessing method.
	void removeReinsertVar(RemoveReinsertType rrOption) { m_rrOptionVar = rrOption; }

	//! Returns the current setting of the remove-reinsert postprocessing method.
	RemoveReinsertType removeReinsertVar() const { return m_rrOptionVar; }
	 ...
	 */
	void percentMostCrossedFix(double percent) { m_percentMostCrossedFix = percent; }

	//! Returns the current setting of option percentMostCrossed.
	double percentMostCrossedFix() const { return m_percentMostCrossedFix; }
	 ...
	 */
	void percentMostCrossedVar(double percent) { m_percentMostCrossedVar = percent; }
	...
	void statistics(bool b) { m_statistics = b; }

	bool statistics() const { return m_statistics; }
...
private:
	...
	ReturnType doCall(PlanRepLight& pr, const Array<edge>& origEdges, const EdgeArray<int>* costOrig,
			const EdgeArray<bool>* forbiddenEdge, const EdgeArray<uint32_t>* edgeSubGraphs) override;
	...
	void recFlipPref(adjEntry adjP, NodeArray<EmbeddingPreference>& pi_pick,
