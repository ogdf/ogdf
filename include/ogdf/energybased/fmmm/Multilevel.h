/** \file
 * \brief Declaration of class Multilevel (used by FMMMLayout).
 *
...
public:
	...
	void create_multilevel_representations(Graph& G, NodeArray<NodeAttributes>& A,
			...
			int min_Graph_size, int rand_tries, Array<Graph*>& G_mult_ptr,
	...
	void find_initial_placement_for_level(int level,
	...
	void delete_multilevel_representations(Array<Graph*>& G_mult_ptr,
			Array<NodeArray<NodeAttributes>*>& A_mult_ptr,
			Array<EdgeArray<EdgeAttributes>*>& E_mult_ptr, int max_level);
...
private:
	...
	bool edgenumbersum_of_all_levels_is_linear(Array<Graph*>& G_mult_ptr, int act_level,
			int& bad_edgenr_counter);

	//! Sets the default multilevel values for all nodes and edges in G_mult
	void init_multilevel_values(const Graph& G_mult, NodeArray<NodeAttributes>& A_mult,
	...
	//! calculated
	void partition_galaxy_into_solar_systems(Array<Graph*>& G_mult_ptr,
			Array<NodeArray<NodeAttributes>*>& A_mult_ptr,
			Array<EdgeArray<EdgeAttributes>*>& E_mult_ptr, int rand_seed,
			FMMMOptions::GalaxyChoice galaxy_choice, int random_tries, int act_level);
	...
	void create_suns_and_planets(Array<Graph*>& G_mult_ptr,
			Array<NodeArray<NodeAttributes>*>& A_mult_ptr,
			Array<EdgeArray<EdgeAttributes>*>& E_mult_ptr, int rand_seed,
			FMMMOptions::GalaxyChoice galaxy_choice, int random_tries, int act_level);

	//! Partitions the nodes of G_mult that have not been assigned yet,
	//! to moon nodes of a nearest planet or pm node and identify this planet as a
	//! pm-node if this has not been done before.
	void create_moon_nodes_and_pm_nodes(const Graph& G_mult, NodeArray<NodeAttributes>& A_mult,
			EdgeArray<EdgeAttributes>& E_mult);
	...
	//! values.
	void collaps_solar_systems(Array<Graph*>& G_mult_ptr,
			Array<NodeArray<NodeAttributes>*>& A_mult_ptr,
	...
	void calculate_mass_of_collapsed_nodes(Array<Graph*>& G_mult_ptr,
	...
	void create_edges_edgedistances_and_lambda_Lists(Array<Graph*>& G_mult_ptr,
			Array<NodeArray<NodeAttributes>*>& A_mult_ptr,
	...
	void delete_parallel_edges_and_update_edgelength(Array<Graph*>& G_mult_ptr,
			Array<EdgeArray<EdgeAttributes>*>& E_mult_ptr, EdgeArray<double>& new_edgelength,
			int act_level);

	//! The initial positions of all sun_nodes at level level are set.
	void set_initial_positions_of_sun_nodes(int level, Array<Graph*>& G_mult_ptr,
			Array<NodeArray<NodeAttributes>*>& A_mult_ptr);

	//! The initial positions of the planet/moon_nodes at level level are calculated here
	//! and a list of all pm_nodes is returned.
	void set_initial_positions_of_planet_and_moon_nodes(int level,
			FMMMOptions::InitialPlacementMult init_placement_way, Array<Graph*>& G_mult_ptr,
			Array<NodeArray<NodeAttributes>*>& A_mult_ptr,
	...
	//! at multilevel level.
	void create_all_placement_sectors(Array<Graph*>& G_mult_ptr,
			Array<NodeArray<NodeAttributes>*>& A_mult_ptr,
	...
	void set_initial_positions_of_pm_nodes(int level,
