...
private:
    ...
    bool halldorssonRecursive(const Graph& graph, List<node>& independentSet, int k, double alpha) {
        ...
        unsigned int m =
          std::max(std::min(std::floor(alpha * (std::log(n) / std::log(k))), static_cast<double>(n)), 1.0);
        Array<Array<node>> buckets;
        createBuckets(graphMain, std::min(k * m, n), buckets);
        for (Array<node>& bucket : buckets) {
            for (NodeColoringHalldorsson::SubsetIterator subsetIterator(bucket, m); subsetIterator.isOk();
                ...
                if (checkIndependentSet<ListIterator<node>>(graphMain, subset.begin())) {
                    ...
                    getNeighborsComplement<ListIterator<node>>(graphMain, subset.begin(), neighborsComplement);
                    double size =
                      static_cast<double>(n) / static_cast<double>(k) * std::log(n) / (2.0 * std::log(std::log(n)));
                    ...
                    inducedSubGraph<ListIterator<node>>(
                      graphMain, neighborsComplement.begin(), subGraph, nodeTableOrig2New);
                    ...
                    if (neighborsComplement.size() >= size) {
                        ...
                        int threshold = std::ceil(std::pow(std::log(n), 3.0) / (6.0 * std::log(std::log(n))));
    ...
    struct SearchWrapperHalldorsson : public SearchWrapper {
         ...
         */
        SearchWrapperHalldorsson(NodeColoringHalldorsson& coloringHalldorsson,
          const Graph& graph,
        ...
        bool step(int k) override { return m_coloring.halldorssonRecursive(m_graph, m_independentSet, k, m_alpha); }
