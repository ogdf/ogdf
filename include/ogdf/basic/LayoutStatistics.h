...
public:
	...
	//! Computes the angular resolution of the layout given by \p ga.
	/**
	 ...
	 *                         \p pMaxAngle may be a \c nullptr in which case nothing is stored.
	 * \param pAvgAngle points to a variable in which the average angle in the layout is stored;
	 *                         \p pAvgAngle may be a \c nullptr in which case nothing is stored.
	...
	//! Computes the number of edge crossings in the layout given by \p ga.
	/**
	 ...
	 * The edges corrsepond to edges in the input layout: If an edge connecting points \a p and \a q in \p H corresponds
	 ...
	 *
	 * To put it more simple, we obtain graph \p H from the input layout \p ga by putting a dummy vertex on each crossing
	 ...
	 * \param origNode  maps nodes in \p H to nodes in the input graph (given by \p ga); points which are only intersection points of segments are mapped to \c nullptr.
	 * \param origEdge  maps edges in \p H to the corresponding edges in the input graph (given by \p ga).
	 */
