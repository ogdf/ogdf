...
public:
	...
	/**
	 * Constructs a new PCTree that is part of \p p_forest and can thus be merged with any other PCTree of the same forest.
	 */
	explicit PCTree(PCTreeForest* p_forest) {
		if (p_forest) {
			forest = p_forest;
			externalForest = true;
		} else {
			forest = new PCTreeForest();
			externalForest = false;
		}
	};
	 ...
	 * @param p_forest Automatically create and manages a forest if \p p_forest is null.
	 */
	explicit PCTree(const PCTree& other, PCTreeNodeArray<PCNode*>& nodeMapping,
			bool keep_ids = false, PCTreeForest* p_forest = nullptr);

	/**
	 * Deserialize a PCTree from a string \p str as generated by operator<<(std::ostream&, const PCTree*) or PCTree::uniqueID().
	 * Automatically creates and manages a forest if \p p_forest is null.
	 */
	explicit PCTree(const std::string& str, bool keep_ids = false, PCTreeForest* p_forest = nullptr);
