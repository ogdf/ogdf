...
class PCTree {
 ...
 public:
  ...
  explicit PCTree(
	  const PCTree &other, PCTreeNodeArray<PCNode *> &nodeMapping, bool keep_ids = false);
 ...
 public: // Getters
  operator const PCTreeRegistry<PCNode *> &() const {
	  return forest->nodeArrayRegistry;
  }

  [[nodiscard]] PCTreeForest *getForest() const {
	  return forest;
  }

  [[nodiscard]] bool isTrivial() const;

  [[nodiscard]] const IntrusiveList<PCNode> &getLeaves() const {
	  return leaves;
  }
  ...
  [[nodiscard]] size_t getPNodeCount() const {
	  return pNodeCount;
  }

  [[nodiscard]] size_t getCNodeCount() const {
	  return cNodeCount;
  }

  [[nodiscard]] PCNode *getRootNode() const {
	  return rootNode;
  }

  int getTerminalPathLength() const {
	  return terminalPathLength;
  }
  ...
  void getRestrictions(
	  std::vector <std::vector<PCNode *>> &restrictions, PCNode *fixedLeaf = nullptr) const;
  ...
  std::string uniqueID(const std::function<void(std::ostream & os, PCNode * , int)> &printNode =
  uid_utils::nodeToID,
 ...
 public: // Observers
  ...
  struct Observer {
	...
	virtual void terminalPathFound([[maybe_unused]] PCTree &tree,
	                               [[maybe_unused]] PCNode *apex, [[maybe_unused]] PCNode *apexTPPred2,
	                               [[maybe_unused]] int terminalPathLength) {};

	virtual void centralCreated(
		[[maybe_unused]] PCTree &tree, [[maybe_unused]] PCNode *central) {};

	virtual void beforeMerge([[maybe_unused]] PCTree &tree, [[maybe_unused]] int count,
	                         [[maybe_unused]] PCNode *tpNeigh) {};

	virtual void afterMerge([[maybe_unused]] PCTree &tree,
	                        [[maybe_unused]] PCNode *successor, [[maybe_unused]] PCNode *mergedNode) {};

	virtual void whenPNodeMerged([[maybe_unused]] PCTree &tree,
	                             [[maybe_unused]] PCNode *tpNeigh, [[maybe_unused]] PCNode *tpPred,
	...
	virtual void fullNodeSplit(
		[[maybe_unused]] PCTree &tree, [[maybe_unused]] PCNode *fullNode) {};
	...
	virtual void nodeDeleted(
		[[maybe_unused]] PCTree &tree, [[maybe_unused]] PCNode *toBeDeleted) {};
	virtual void nodeReplaced([[maybe_unused]] PCTree &tree,
	                          [[maybe_unused]] PCNode *replaced, [[maybe_unused]] PCNode *replacement) {};
  };
...
int PCTreeRegistry<Key>::keyArrayTableSize() const {
	return ogdf::RegistryBase<Key, PCTreeRegistry<Key>>::calculateTableSize(
		m_pForest->nextNodeId);
}
