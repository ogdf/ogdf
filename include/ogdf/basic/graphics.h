 ...
 * @ingroup graph-drawing
 */
enum class StrokeLineCap : unsigned char { Butt, Round, Square };
 ...
 * @ingroup graph-drawing
 */
enum class StrokeLineJoin : unsigned char { Miter, Round, Bevel };
...
//! Output operator
OGDF_EXPORT std::ostream& operator<<(std::ostream& os, const Shape& shape);
...
public:
	...
	Color(int r, int g, int b, int a = 255)
		: m_red((uint8_t)r), m_green((uint8_t)g), m_blue((uint8_t)b), m_alpha((uint8_t)a) { }
	...
	//! Sets this color to the color defined by \p str.
	/**
	 * @param str string of the form "#rgb" or "#rrggbb"
	 * @return true iff the conversion succeeded (i.e., the string was a valid
	 * color string of the form "#rgb" or "#rrggbb"). If the conversion did not
	 * succeed, false is returned, and this Color is set to black.
	 */
	...
	friend std::ostream& operator<<(std::ostream& os, const Color& c) { return os << c.toString(); }
};
...
struct OGDF_EXPORT Stroke {
	...
	Stroke()
		: m_color(Color::Name::Black)
		, m_width(1.0f)
		, m_type(StrokeType::Solid)
		, m_cap(StrokeLineCap::Butt)
		, m_join(StrokeLineJoin::Miter) { }
...
struct OGDF_EXPORT Fill {
	...
	Fill()
		: m_color(Color::Name::White), m_bgColor(Color::Name::Black), m_pattern(FillPattern::Solid) { }

	Fill(Color c) : m_color(c), m_bgColor(Color::Name::Black), m_pattern(FillPattern::Solid) { }

	Fill(Color c, FillPattern pattern)
		: m_color(c), m_bgColor(Color::Name::Black), m_pattern(pattern) { }

	Fill(Color c, Color bgColor, FillPattern pattern)
		: m_color(c), m_bgColor(bgColor), m_pattern(pattern) { }
};
