 ...
 * last checkin:
 *   $Author: gutwenger $
 *   $Date: 2012-12-11 16:43:11 +0100 (Di, 11. Dez 2012) $
 ***************************************************************/
...
namespace ogdf {
				...
				for (i = 0; i < elNumLabels; i++)
				{
					...
					if (!(m_assigned[i])[e]) //assign intersection free
					{
						...
						if (m_candPosList[i][e].size() > 0)
						{
							...
							//check: elNumLabels <4 => ln-2 <=1 should not happen
							if ((i == 0) || (i == 1)) preferredIndex = 0;
							if ( ((i == elNumLabels-2) || (i == elNumLabels-1))  && (m_candPosList[i][e].size() > 0) )
	...
	void ELabelPos<coordType>::computeCandidates()
	{
		...
		forall_nodes(v, *m_prup)
		{
			...
			forall_adj_edges(e, w)
			{
				...
				case 1 :
					{
						...
						//elNumLabels is the OGDF-wide number of possible label types
						...
						//candidates
						double distanceStep = m_edgeLength[e] / (elNumLabels*m_posNum);
						...
						while (currentPos <= elNumLabels*m_posNum)
						{
#ifdef foutput
							//fout<<"  segment:\n";
							...
							while ((distance < currentEnd)
								&& (currentPos <= elNumLabels*m_posNum))
							{
								...
								case elNumLabels-2: symmLabel = elNumLabels-1; break;
								case elNumLabels-1: symmLabel = elNumLabels-2; break;
								}//switch
								...
								case odWest:  //horizontal west
									{
										...
										case 1: symmCandidate.m_y = candidate.m_y +
											...
											symmCandidate.m_x = candidate.m_x;
											break;
										case elNumLabels-2: symmCandidate.m_y = candidate.m_y -
															 (m_distance[symmLabel][e] + symmH);
											candidate.m_y += (m_distance[labelIndex][e] + alhHalf);
											symmCandidate.m_x = candidate.m_x;
											break;
										case elNumLabels-1: symmCandidate.m_y = candidate.m_y +
															 (m_distance[symmLabel][e] + symmH);
											candidate.m_y -= (m_distance[labelIndex][e] + alhHalf);
											symmCandidate.m_x = candidate.m_x;
											break;
								...
								case odNorth:
									{
										...
										case 1: symmCandidate.m_x = candidate.m_x -
											...
											symmCandidate.m_y = candidate.m_y;
											break;
										case elNumLabels-2: symmCandidate.m_x = candidate.m_x +
															 (m_distance[symmLabel][e] + symmW);
											candidate.m_x -= (m_distance[labelIndex][e] + alwHalf);
											symmCandidate.m_y = candidate.m_y;
											break;
										case elNumLabels-1: symmCandidate.m_x = candidate.m_x -
															 (m_distance[symmLabel][e] + symmW);
											candidate.m_x += (m_distance[labelIndex][e] + alwHalf);
											symmCandidate.m_y = candidate.m_y;
											break;
								...
								if ( (labelIndex == 0) ||
									(labelIndex == 1) ||
									(labelIndex == elNumLabels-2) ||
									(labelIndex == elNumLabels-1)
									)
								{
						...
						//for (labelIndex = 0; labelIndex < elNumLabels; labelIndex++)
				...
				int forI;
				for(forI = 0; forI < elNumLabels; forI++)
				{
	...
	void ELabelPos<coordType>::initStructure()
	{
		...
		EdgeArray<node> l_pairNode[elNumLabels];
		//use same order in candidate list to navigate in l_posNode
		EdgeArray< List<node> > l_posNode[elNumLabels];
	...
	void ELabelPos<coordType>::testFeatureIntersect()
	{
		...
		EdgeArray< GenericPoint<coordType> > l_saveCandidate[elNumLabels];
		for (int k = 0; k < elNumLabels; k++) l_saveCandidate[k].init(m_prup->original());
	...
	template <class coordType>
	void ELabelPos<coordType>::saveRecovery(EdgeArray< GenericPoint<coordType> > (&saveCandidate)[elNumLabels])
	{
	...
	void ELabelPos<coordType>::call(GraphAttributes& ug, ELabelInterface<coordType>& eli) //double
	{
		...
		EdgeArray< List< PosInfo* > > l_freeCand[elNumLabels];
		for (int k=0; k < elNumLabels; k++)
		{
				...
				for (i = 0; i < elNumLabels; i++)
				{
					...
					if (!(m_assigned[i])[e]) //assign intersection free
					{
						...
						if (l_freeCand[i][e].size() > 0)
						{
							...
							if ((i == 0) || (i == 1)) preferredIndex = 0;
							if ( ((i == elNumLabels-2) || (i == elNumLabels-1))  && (l_freeCand[i][e].size() > 0) )
			...
			case 1 :
				{
					...
					coordType standardDist = //max(m_eli->minFeatDist(),  //max/min!!??
						coordType((m_edgeLength[e] / (elNumLabels*m_posNum + 1)));
					...
					//all labeltypes*the number of cand. per labeltype
					while (currentPos <= elNumLabels*m_posNum) //todo: labelnumber sum
					{
						int segStartLabel = currentPos-1;
						//stay within current segment
							...
							&& (currentPos <= elNumLabels*m_posNum))
						{
							...
							else if ( (labelIndex == elNumLabels-1) || (labelIndex == elNumLabels-2))
								optPosDistance = elNumLabels*m_posNum - currentPos;
	...
	void ELabelPos<coordType>::testUMLFeatureIntersect()
	{
		...
		for (int k = 0; k < elNumLabels; k++) l_saveCandidate[k].init(m_ug->constGraph());
		...
		(&saveCandidate)[elNumLabels])
	{
		...
		forall_edges(e, mapGraph)
		{
			...
			for (i = 0; i < elNumLabels; i++)
			{
				...
				else if ((i == elNumLabels-1) || (i == elNumLabels-2))
