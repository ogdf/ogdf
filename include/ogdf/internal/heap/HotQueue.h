...
struct HotQueueNode {
	...
	HotQueueNode(const V &value, const P &priority)
...
private:
	...
	HotQueueHandle(std::size_t index, HotQueueNode<V, P> *node)
	: type(Type::bucket), bucketHandle(index, node)
	{
	}
	...
	const V &value, const P &priority)
{
	...
	if(ind == m_heapedBucket) {
		...
		HotQueueNode<V, P> *node = new HotQueueNode<V, P>(value, priority);

		if(bucketAt(ind) != nullptr) {
			bucketAt(ind)->prev = node;
		}
		node->next = bucketAt(ind);
		bucketAt(ind) = node;

		m_lastBucket = std::max(m_lastBucket, ind);
		return Handle(ind, node);
	}
}
	...
	const P &priority)
{
	...
	case Handle::Type::bucket:
		...
		HotQueueNode<V, P> *node = std::get<1>(handle.bucketHandle);
		...
		if(node->prev != nullptr) {
			...
			m_buckets[std::get<0>(handle.bucketHandle)] = node->next;
		}

		// ... and push it again with new priority, releasing the memory.
		m_size--;
		handle = push(node->value, priority);
		delete node;
		break;
	}
}
