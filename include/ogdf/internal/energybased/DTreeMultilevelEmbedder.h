...
void DTreeMultilevelEmbedder<Dim>::call(const Graph& graph, NodeArray<NodeCoords>& resultCoords)
{
    ...
    for (GalaxyLevel* pCurrLevel = pLevelEnd; pCurrLevel; pCurrLevel = pCurrLevel->nextFiner())
    {
        ...
        DTreeEmbedder<Dim>* pEmbedder = new DTreeEmbedder<Dim>(pCurrLevel->graph());
        ...
        if (pCurrLevel->isCoarsestLevel())
        {
            int numIterations = std::min(std::max(m_minIterationsPerLevel, (int)m_numIterationsCoarsestLevel), m_maxIterationsPerLevel);
        ...
        {
            int numIterations = std::min(std::max(m_minIterationsPerLevel, (int)currNumIterations), m_maxIterationsPerLevel);
            pEmbedder->scaleNodes(3.0);
            pEmbedder->doIterationsNewton(numIterations, currThreshold, RepForceFunctionNewton<Dim, 1>, AttrForceFunctionPow<Dim, 2>);
            pEmbedder->scaleNodes(1.0 / 3.0);
            pEmbedder->doIterationsNewton(numIterations, currThreshold, RepForceFunctionNewton<Dim, 2>, AttrForceFunctionPow<Dim, 2>);
