...
public:
	...
	inline void setLevel(NodeID node, uint32_t level)
	{
	...
	inline uint32_t numberOfChilds(NodeID node) const
	{
		return m_tree[node].numChilds;
	}

	//! sets the number of children of a node
	inline void setNumberOfChilds(NodeID node, uint32_t numChilds)
	{
		m_tree[node].numChilds = numChilds;
	}

	//! returns the i th child index of node node
	inline NodeID child(NodeID node, uint32_t i) const
	{
		return m_tree[node].child[i];
	}

	//! sets the i-th child index of node node
	inline void setChild(NodeID node, uint32_t i, NodeID c)
	{
	...
	inline uint32_t numberOfPoints(NodeID node) const
	{
		return m_tree[node].numPoints;
	}

	//! sets the number of nodes containted in node
	inline void setNumberOfPoints(NodeID node, uint32_t numPoints)
	{
...
private:
	...
	inline void initInnerNode(NodeID node, NodeID leftChild, NodeID rightChild, uint32_t level, NodeID next)
	{
