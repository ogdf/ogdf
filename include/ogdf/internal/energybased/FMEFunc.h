...
public:
	...
	void newPartition(uint32_t node)
	{
		...
		if (tree->isLeaf(node) || (tree->numberOfPoints(node) < bound))
			...
			for (uint32_t i = 0; i < tree->numberOfChilds(node); i++)
...
public:
	...
	inline void operator()(uint32_t i)
	{
		uint32_t node = quadtree->directNode(i);
		uint32_t offset = quadtree->firstPoint(node);
		uint32_t numPoints = quadtree->numberOfPoints(node);
...
public:
	...
	inline void operator()(uint32_t i)
	{
		...
		{
			uint32_t l = (globalContext->pQuadtree->refOfPoint(i));
			if (FLAGS & COLLECT_REPULSIVE_FACTOR)
			{
