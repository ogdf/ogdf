/** \file
 * \brief Declaration of the sub-problem class for the Branch&Cut&Price algorithm
 * for the c-planarity problem.
...
private:
	...
	// induced by \a it and given support graph \a gc. Returns list of node pairs that
	// correspond to connection edges in parameter \a subDivOrig.
	KuraSize subdivisionLefthandSide(
	...
	{
		return separateCutPool(static_cast<CPlanarityMaster*>(master_)->getCutConnPool(),minViolation);
	}
	//tries to regenerate kuratowski cuts
	inline int separateKuraPool(double minViolation)
	{
		return separateCutPool(static_cast<CPlanarityMaster*>(master_)->getCutKuraPool(),minViolation);
	}
