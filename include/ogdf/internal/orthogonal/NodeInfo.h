 ...
 * last checkin:
 *   $Author: gutwenger $
 *   $Date: 2012-10-13 11:58:23 +0200 (Sa, 13. Okt 2012) $
 ***************************************************************/
...
public:
	...
	int nodeSize(OrthoDir od) const { return ((od % 2 == 0) ? box_y_size : box_x_size); }
	int cageSize(OrthoDir od) const { return ((od % 2 == 0) ? cage_y_size : cage_x_size); }
	...
	int num_bend_edges(OrthoDir s1, OrthoDir sneighbour) { return m_nbe[s1][sneighbour]; }
	int num_E_hook(OrthoDir s1, OrthoDir sneighbour) { return m_routable[s1][sneighbour]; }
