...
void EmbedderMinDepthPiTa::doCall(Graph& G, adjEntry& adjExternal) {
	...
	if (Tdiam_initialized) {
		...
		for (adjEntry adj : rootOfBlockCutfaceTree->adjEntries) {
			...
			node bPG = computeBlockMapping(nbDG, rootOfBlockCutfaceTree, tmpBlocksNodes,
					tmpChildBlocks);
			bDG_to_bPG[nbDG] = bPG;
			bPG_to_bDG[bPG] = nbDG;
		}
		...
		if (m_blockCutfaceTree.typeOfBNode(m_knot) == BCTree::BNodeType::BComp
			...
			for (face f : CE.faces) {
				...
				for (adjEntry ae : f->entries) {
					node orgNode = pBCTree->original(
							nBlockEmbedding_to_nH[bDG_to_bPG[m_knot]][ae->theNode()]);
				...
				if (numOfEntriesFromList == childrenOfKnot.size()) {
					...
					for (ListIterator<node> it = childrenOfKnot.begin(); it.valid(); ++it) {
						...
						for (ListIterator<adjEntry> it_cpy = adjList[bT][nG].begin();
								it_cpy.valid(); ++it_cpy) {
...
node EmbedderMinDepthPiTa::computeBlockMapping(const node& bDG, const node& parent,
		List<node>& blocksNodes, List<node>& childBlocks) {
	...
	embedder::ConnectedSubgraph<int>::call(pm_blockCutfaceTree->auxiliaryGraph(), SG, parentH,
			nSG_to_nH);
	...
	for (node nSG : SG.nodes) {
		...
		for (ListIterator<adjEntry> it = beginIt; it.valid(); ++it) {
			node nPG = (*it)->theNode();
			if (!childNodes.search(nPG).valid() && !blockNodesDG.search(nPG).valid()
...
void EmbedderMinDepthPiTa::embedBlocks(const node& bT, const node& cH) {
	...
	if (!useExtendedDepthDefinition() && blockG[bT]->numberOfEdges() == 1) {
		for (node n : blockG[bT]->nodes) {
...
void EmbedderMinDepthPiTa::embedBlockVertex(const node& bT, const node& parent_cT) {
	...
	for (node n_blockG_bT : blockG[bTp]->nodes) {
		...
		if (pBCTree->typeOfGNode(nG) == BCTree::GNodeType::CutVertex) {
			...
			if (cTp != nBCTree_to_npBCTree[parent_cT]) {
				...
				for (node n_G_cT : G_nT[cTp]->nodes) {
					...
					bool after_ae = true;
					for (adjEntry aeNode = adjE; after_ae || aeNode != adjE;
							after_ae = after_ae && aeNode->succ(),
		...
		bool after_ae = true;
		for (adjEntry aeNode = ae; after_ae || aeNode != ae; after_ae = after_ae && aeNode->succ(),
