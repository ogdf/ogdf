/** \file
 * \brief Implementation of the PlanarSubgraphFast.
 *
...
// default constructor
PlanarSubgraphFast::PlanarSubgraphFast()
{
	m_nRuns = 10;
}
...
// copy constructor
PlanarSubgraphFast::PlanarSubgraphFast(const PlanarSubgraphFast &fps) : PlanarSubgraphModule(fps)
{
	m_nRuns = fps.m_nRuns;
}
...
// clone method
PlanarSubgraphFast *PlanarSubgraphFast::clone() const
{
	return new PlanarSubgraphFast(*this);
}


// assignment operator
PlanarSubgraphFast &PlanarSubgraphFast::operator=(const PlanarSubgraphFast &fps)
{
...
class PlanarSubgraphFast::ThreadMaster {
...
class PlanarSubgraphFast::Worker {
...
PlanarSubgraphFast::ThreadMaster::ThreadMaster(const Array<BlockType> &block, const EdgeArray<int> *pCost, int runs)
...
List<edge> *PlanarSubgraphFast::ThreadMaster::postNewResult(int i, List<edge> *pNewDelEdges)
{
	int newSolution;
	if(m_pCost == nullptr) {
...
void PlanarSubgraphFast::ThreadMaster::buildSolution(List<edge> &delEdges)
{
	for(int i = 0; i < m_nBlocks; ++i) {
...
void PlanarSubgraphFast::doWorkHelper(ThreadMaster &master)
{
...
Module::ReturnType PlanarSubgraphFast::doCall(
	const Graph &G,
...
//
void PlanarSubgraphFast::seqCall(const Array<BlockType> &block, const EdgeArray<int> *pCost, int nRuns, bool randomize, List<edge> &delEdges)
{
...
//
void PlanarSubgraphFast::parCall(const Array<BlockType> &block, const EdgeArray<int> *pCost, int nRuns, unsigned int nThreads, List<edge> &delEdges)
{
...
void PlanarSubgraphFast::planarize(
	const Graph &G,
	...
	List<edge> &delEdges)
{
	...
	for(node v : G.nodes)
	{
		for(adjEntry adj : v->adjEntries) {
			...
			{
				PlanarLeafKey<whaInfo*>* L = new PlanarLeafKey<whaInfo*>(e);
