...
#include <ogdf/basic/SList.h>                                          // for SList, SListIterat...
#include <ogdf/decomposition/BCTree.h>                                 // for BCTree
#include <ogdf/decomposition/StaticSPQRTree.h>                         // for StaticSPQRTree
#include <ogdf/planarity/EmbedderMinDepthMaxFace.h>                    // for EmbedderMinDepthMa...
#include <ogdf/planarity/embedder/ConnectedSubgraph.h>                 // for ConnectedSubgraph
#include <ogdf/planarity/embedder/EmbedderMaxFaceBiconnectedGraphs.h>  // for EmbedderMaxFaceBic...
...
void EmbedderMinDepthMaxFace::doCall(Graph& G, adjEntry& adjExternal) {
	...
	for (node bT : pBCTree->bcTree().nodes) {
		...
		if (minDepth[bT] < d_opt || (minDepth[bT] == d_opt && maxFaceSize[bT] > ell_opt)) {
			d_opt = minDepth[bT];
...
int EmbedderMinDepthMaxFace::bottomUpTraversal(const node& bT, const node& cH) {
	...
	int cstrLength_B_c = EmbedderMaxFaceBiconnectedGraphs<int>::computeSize(blockGraph_bT,
			cInBlockGraph_bT, nodeLengthSG, zeroEdgeLength);
...
void EmbedderMinDepthMaxFace::topDownTraversal(const node& bT) {
	...
	embedder::ConnectedSubgraph<int>::call(pBCTree->auxiliaryGraph(), blockGraph_bT,
			(*(pBCTree->hEdges(bT).begin()))->source(), m_nodeLength, nodeLengthSG, nG_to_nSG);
	...
	if (!blockGraph_bT.empty() && blockGraph_bT.numberOfNodes() != 1
	...
	int cstrLength_B_c = EmbedderMaxFaceBiconnectedGraphs<int>::computeSize(blockGraph_bT,
			nodeLengthSG, edgeLengthBlock, spqrTree, edgeLengthSkel);
	...
	if (md_M_B[bT].size() > 0) {
		...
		for (adjEntry adj : bT->adjEntries) {
			...
			if (md_M_B[bT].size() == 1 && cT1 == cT) {
				...
				for (adjEntry adjBT : bT->adjEntries) {
					...
					for (adjEntry adjCT2 : cT2->adjEntries) {
						...
						} else if (m2 == cB[e_cT2_bT2]
								&& !M2[bT].search(pBCTree->cutVertex(cT2, bT)).valid()) {
							M2[bT].pushBack(pBCTree->cutVertex(cT2, bT));
						}
					}
				}
				...
				int maxFaceSizeInBlock = EmbedderMaxFaceBiconnectedGraphs<int>::computeSize(
						blockGraphBT, cInBlockGraph_bT, nodeLengthSGBT, zeroEdgeLength);
				if (M2[bT].size() == 0) {
					cB[e_bT_cT] = 1;
				} else {
			...
			} else //M_B.size() != 1
			{
				...
				int maxFaceSizeInBlock = EmbedderMaxFaceBiconnectedGraphs<int>::computeSize(
						blockGraph_bT, cInBlockGraph_bT, nodeLengthSG, edgeLengthBlock, spqrTree,
						edgeLengthSkel);
				if (md_M_B[bT].size() == 0) {
					cB[e_bT_cT] = 1;
				} else {
		...
		for (adjEntry adj : bT->adjEntries) {
			...
			for (adjEntry adjCT : cT->adjEntries) {
				...
				if (m_B < cB[e_cT_bT2]) {
					...
					md_M_B[bT].clear();
					md_M_B[bT].pushBack(pBCTree->cutVertex(cT, bT));
				} else if (m_B == cB[e_cT_bT2]
						&& !md_M_B[bT].search(pBCTree->cutVertex(cT, bT)).valid()) {
					md_M_B[bT].pushBack(pBCTree->cutVertex(cT, bT));
				}
			}
		}
...
int EmbedderMinDepthMaxFace::constraintMaxFace(const node& bT, const node& cH) {
	...
	int cstrLengthBc = EmbedderMaxFaceBiconnectedGraphs<int>::computeSize(blockGraph, cInBlockGraph,
...
void EmbedderMinDepthMaxFace::maximumFaceRec(const node& bT, node& bT_opt, int& ell_opt) {
	...
	internalMaximumFaceRec(
			bT, bT_opt, ell_opt, blockGraph_bT, nodeLengthSG, spqrTree,
			[&](node cH) -> node& { return nG_to_nSG[cH]; },
			[&](node v, node u) -> int& { return mf_cstrLength[u]; },
			[&](node v, node u) -> int& { return mf_nodeLength[u]; }, &maxFaceSize[bT]);

	delete spqrTree;
}

void EmbedderMinDepthMaxFace::embedBlock(const node& bT, const node& cT,
	...
	internalEmbedBlock(bT, cT, after, SG, nodeLengthSG, edgeLengthSG, nSG_to_nG, eSG_to_eG,
