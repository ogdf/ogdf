...
#include <ogdf/basic/CombinatorialEmbedding.h>                         // for CombinatorialEmbed...
...
#include <ogdf/planarity/EmbedderMinDepth.h>                           // for EmbedderMinDepth
		...
		for (adjEntry adjCT : cT->adjEntries) {
			...
			if (m_B < m_cB[e_cT_bT2]) {
				...
				cInBWithProperty.pushBack(cV_in_bT);
					...
					&& !cInBWithProperty.search(pBCTree->cutVertex(cT, bT)).valid()) {
				node cV_in_bT = pBCTree->cutVertex(cT, bT);
				cInBWithProperty.pushBack(cV_in_bT);
			}
		}
			...
			for (adjEntry adjCT : cT->adjEntries) {
				...
				if (m_B < m_cB[e_cT_bT2]) {
					...
					M_B[bT].pushBack(pBCTree->cutVertex(cT, bT));
						...
						&& !M_B[bT].search(pBCTree->cutVertex(cT, bT)).valid()) {
					M_B[bT].pushBack(pBCTree->cutVertex(cT, bT));
				}
			}
		}
...
void EmbedderMinDepth::embedBlock(const node& bT, const node& cT, ListIterator<adjEntry>& after) {
	...
	if (*pAdjExternal == nullptr) {
		...
		for (adjEntry ae = ae1; ae; ae = ae->succ()) {
			if (ae->theEdge()
					== pBCTree->original(eBlockEmbedding_to_eH[bT][m_adjExternal->theEdge()])) {
