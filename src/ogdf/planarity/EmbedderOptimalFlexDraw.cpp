...
#include <ogdf/graphalg/MinCostFlowModule.h>          // for MinCostFlowModule
#include <ogdf/graphalg/MinCostFlowReinelt.h>         // for MinCostFlowReinelt
#include <ogdf/planarity/EmbedderOptimalFlexDraw.h>   // for EmbedderOptimalFlexDraw
...
void EmbedderOptimalFlexDraw::createNetwork(node parent, node mu, int bends, NodeArray<int> cost[],
		Skeleton& skeleton, EdgeArray<node>& edgeNode, Graph& N, EdgeArray<int>& upper,
		EdgeArray<int>& perUnitCost, NodeArray<int>& supply) {
...
void EmbedderOptimalFlexDraw::optimizeOverEmbeddings(StaticPlanarSPQRTree& T, node parent, node mu,
		int bends, NodeArray<int> cost[], NodeArray<long long> embedding[]) {
	...
	for (long long currentEmbedding = 0; currentEmbedding < embeddingsCount; ++currentEmbedding) {
		...
		createNetwork(parent, mu, bends, cost, skeleton, edgeNode, N, upper, perUnitCost, supply);
...
void EmbedderOptimalFlexDraw::computePrincipalSplitComponentCost(StaticPlanarSPQRTree& T,
		NodeArray<int> cost[], NodeArray<long long> embedding[], node parent, node mu) {
	for (adjEntry adj = mu->firstAdj(); adj != nullptr; adj = adj->succ()) {
...
void EmbedderOptimalFlexDraw::doCall(Graph& G, adjEntry& adjExternal) {
	...
	createNetwork(nullptr, minCostRoot, 0, cost, skeleton, edgeNode, N, upper, perUnitCost, supply);
	...
	for (node mu = T.tree().firstNode(); mu != nullptr; mu = mu->succ()) {
		...
		int bends = 0;
		for (adjEntry adj = edgeNode[T.skeleton(mu).referenceEdge()]->firstAdj(); adj != nullptr;
				adj = adj->succ()) {
