...
#include <math.h>                                // for asin, cos, sin, sqrt
...
#include <ogdf/upward/DominanceLayout.h>         // for DominanceLayout
...
void DominanceLayout::compact(const UpwardPlanRep& UPR, GraphAttributes& GA) {
	...
	while (!xNodes.empty()) {
		...
		if ((yPreCoord[v] > yPreCoord[u])
				|| (firstout[v] == lastout[v] && firstin[u] == lastin[u] && m_L <= m_R)) {
...
void DominanceLayout::findTransitiveEdges(const UpwardPlanRep& UPR, List<edge>& edges) {
	...
	for (face f : UPR.getEmbedding().faces) {
		...
		for (adjEntry adj : f->entries) {
			...
			if ((adj->faceCycleSucc()->theEdge()->source() == src
						&& adj->faceCyclePred()->theEdge()->target() == tgt)
					|| (adj->faceCycleSucc()->theEdge()->target() == tgt
							&& adj->faceCyclePred()->theEdge()->source() == src)) {
