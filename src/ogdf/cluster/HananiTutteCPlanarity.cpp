...
namespace ogdf {
	...
	public:
		...
		struct Object {
			...
			// default constructor
			Object() :
				m_t(Type::vertex), m_st(SubType::vertex), m_v(nullptr), m_e(nullptr), m_c(nullptr), m_c2(nullptr) { }

			// type v/v
			Object(node v) :
				m_t(Type::vertex), m_st(SubType::vertex), m_v(v), m_e(nullptr), m_c(nullptr), m_c2(nullptr) { }

			// type e/e
			Object(edge e) :
				m_t(Type::edge), m_st(SubType::edge), m_v(nullptr), m_e(e), m_c(nullptr), m_c2(nullptr) { }

			// type v/c
			Object(cluster c) :
				m_t(Type::vertex), m_st(SubType::cluster), m_v(nullptr), m_e(nullptr), m_c(c), m_c2(nullptr) { }
			...
			Object(node v, cluster c, edge e) :
				m_t(Type::edge), m_st(SubType::vertexCluster), m_v(v), m_e(e), m_c(c), m_c2(nullptr) { }

			// type e/cc
			Object(cluster c, cluster c2, edge e) :
				m_t(Type::edge), m_st(SubType::clusterCluster), m_v(nullptr), m_e(e)
			{
				if(c->index() < c2->index()) {
					m_c = c;  m_c2 = c2;
				} else {
	...
	{
		OGDF_ASSERT(eo->m_t == Type::edge);
		OGDF_ASSERT(obj.m_t == Type::vertex);
	...
	cluster HananiTutteCPlanarity::CGraph::clusterOfEdge(const CLinearSystem::Object &obj, cluster &cl2) const
	{
		...
		case SubType::vertexCluster:
			return m_cg.clusterOf(obj.m_v);

		case SubType::clusterCluster:
			{
				cluster c1 = obj.m_c;
				cluster c2 = obj.m_c2;
		...
		case SubType::innerCluster:
			return obj.m_c;

		case SubType::outerCluster:
			{
		...
		case SubType::crossCluster:
			cl2 = obj.m_c->parent();
			return obj.m_c;

		case SubType::vertex:
		case SubType::cluster:
			return nullptr;
		}
		...
		case SubType::vertexCluster:
			{
				...
				SubType st = (m_cg.clusterOf(u) == c) ? SubType::innerCluster : SubType::outerCluster;
				obj1 = CLinearSystem::Object(u);
				obj2 = CLinearSystem::Object(Type::vertex, st, c, eo->m_e);
			}
			break;
		...
		case SubType::clusterCluster:
			{
				cluster c1 = eo->m_c;
				cluster c2 = eo->m_c2;
				SubType st = SubType::innerCluster;

				if(c2->parent() == c1)
					swap(c1,c2);
				else if(c1->parent() != c1)
					st = SubType::outerCluster;

				obj1 = CLinearSystem::Object(Type::vertex, SubType::outerCluster, c1, eo->m_e);
				obj2 = CLinearSystem::Object(Type::vertex, st, c2, eo->m_e);
			}
			break;

		case SubType::innerCluster:
			obj1 = CLinearSystem::Object(eo->m_c);
			obj2 = CLinearSystem::Object(Type::vertex, SubType::innerCluster, eo->m_c, eo->m_e);
			break;

		case SubType::outerCluster:
			obj1 = CLinearSystem::Object(eo->m_c);
			obj2 = CLinearSystem::Object(Type::vertex, SubType::outerCluster, eo->m_c, eo->m_e);
			break;

		case SubType::crossCluster:
			obj1 = CLinearSystem::Object(Type::vertex, SubType::innerCluster, eo->m_c, eo->m_e);
			obj2 = CLinearSystem::Object(Type::vertex, SubType::outerCluster, eo->m_c, eo->m_e);
			break;
	...
	bool HananiTutteCPlanarity::CGraph::cAdjacent(const CLinearSystem::Object *eo1, const CLinearSystem::Object *eo2) const
	{
		...
		if(obj11 == obj21 || obj11 == obj22)
			if(obj11.m_t == Type::vertex && obj11.m_st == SubType::cluster)
				return true;
	...
	{
		OGDF_ASSERT(vo1.m_t == Type::vertex);
		OGDF_ASSERT(vo2.m_t == Type::vertex);

		SubType st1 = vo1.m_st, st2 = vo2.m_st;
		OGDF_ASSERT(st1 == SubType::vertex || st1 == SubType::innerCluster || st1 == SubType::outerCluster);
		OGDF_ASSERT(st2 == SubType::vertex || st2 == SubType::innerCluster || st2 == SubType::outerCluster);
		...
		case SubType::innerCluster:
			if (st2 == SubType::vertex) {
				return false;
			} else if (st2 == SubType::innerCluster) {
	...
	{
		OGDF_ASSERT(eo1->m_t == Type::edge);
		OGDF_ASSERT(eo2->m_t == Type::edge);
		...
		switch (st1) {
		case SubType::edge:
			canCross = ( st2 == SubType::edge || st2 == SubType::vertexCluster );
			break;
		case SubType::vertexCluster:
			canCross = ( st2 == SubType::edge || st2 == SubType::vertexCluster || st2 == SubType::clusterCluster );
			break;
		case SubType::clusterCluster:
			canCross = ( st2 == SubType::vertexCluster || st2 == SubType::clusterCluster );
			break;
		default:
			break;
		}
	...
	{
		if(obj.m_t == Type::vertex && obj.m_st == SubType::cluster) {
			if( m_ce2[obj.m_c].search(eo1).valid() ) {
				if(!(eo1->m_t == Type::edge && eo1->m_st == SubType::crossCluster))
					return;
				if( m_ce2[obj.m_c].search(eo2).valid() )
					return;
			}
		}

		if(obj.m_t == Type::vertex &&
			(obj.m_st == SubType::innerCluster || obj.m_st == SubType::outerCluster))
		{
			if(obj.m_st == SubType::innerCluster && !m_ce2[obj.m_c].search(eo1).valid() )
				return;

			cluster p = obj.m_c->parent();
			if(obj.m_st == SubType::outerCluster && (p == nullptr || !m_ce2[p].search(eo1).valid()))
				return;

			// added 5/28
			if(eo1->m_t == Type::edge && eo1->m_st == SubType::edge)
				return;

		// added 5/28
		} else if(obj.m_t == Type::vertex && obj.m_st == SubType::vertex) {
			if(!m_ce2[m_cg.clusterOf(obj.m_v)].search(eo1).valid())
				return;
			if( !(eo1->m_t == Type::edge && (eo1->m_st == SubType::edge || eo1->m_st == SubType::vertexCluster || eo1->m_st == SubType::clusterCluster) ) )
				return;
		}

		// change 5/28: do not allow eo1, o moves if o is v vertex and eo1 is
		// inner cluster or outer cluster edge or if o belongs
		// to a different cluster
		if( eo1->m_t == Type::edge && (eo1->m_st == SubType::innerCluster || eo1->m_st == SubType::outerCluster) ) {
			if(obj.m_t == Type::vertex && (obj.m_st == SubType::vertex || obj.m_st == SubType::cluster))
				return;
			if(obj.m_t == Type::vertex && (obj.m_st == SubType::innerCluster || obj.m_st == SubType::outerCluster)) {
				if(eo1->m_c != obj.m_c || eo1->m_st != obj.m_st)
					return;
			}
		}
		...
		if(eo1->m_t == Type::edge && eo1->m_st == SubType::crossCluster) {
			...
			if(eo1->m_c != obj.m_c)
				return;
			SubType st = eo2->m_st;
			if(!( eo2->m_t == Type::edge &&
				(st == SubType::crossCluster || st == SubType::innerCluster || st == SubType::outerCluster)))
				return;
		}

		if(eo2->m_t == Type::edge && eo2->m_st == SubType::crossCluster) {
			SubType st = eo1->m_st;
			if(eo1->m_t == Type::edge && !(st == SubType::innerCluster || st == SubType::outerCluster || st == SubType::crossCluster))
				return;
			if(eo2->m_c != eo1->m_c)
				return;
		}
	...
	void HananiTutteCPlanarity::CGraph::prepareLinearSystem()
	{
		...
		for(cluster c : m_cg.clusters) {
			...
			for(edge e : m_cbe[c]) {
				m_ls.numOx(CLinearSystem::Object(Type::vertex, SubType::innerCluster, c, e));
				m_ls.numOx(CLinearSystem::Object(Type::vertex, SubType::outerCluster, c, e));
				m_ls.numOx(CLinearSystem::Object(Type::edge,   SubType::outerCluster, c, e));
				m_ls.numOx(CLinearSystem::Object(Type::edge,   SubType::innerCluster, c, e));
				m_ls.numOx(CLinearSystem::Object(Type::edge,   SubType::crossCluster, c, e));
			}
		}
	...
	//--------------------------------------------------------
	// C-planarity test (main function)
	//--------------------------------------------------------
	...
	HananiTutteCPlanarity::Verification HananiTutteCPlanarity::isCPlanar(const ClusterGraph &C, bool doPreproc, bool forceSolver, Solver solver)
	{
		...
		if(forceSolver) {
			...
			case Solver::HananiTutte:
				{
					...
					return (icp == true) ? Verification::cPlanar : Verification::nonCPlanarVerified;
				}
			...
			if(G.empty()) {
				...
				return (ccPlanarityTest.call(H) == true) ? Verification::cPlanarVerified : Verification::nonCPlanarVerified;

			} else if(isPlanar(G) == false) {
