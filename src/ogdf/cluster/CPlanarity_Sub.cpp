...
CPlanaritySub::CPlanaritySub(Master *master) : Sub(master,500,static_cast<CPlanarityMaster*>(master)->m_inactiveVariables.size(),2000,false), detectedInfeasibility(false), inOrigSolveLp(false), bufferedForCreation(10) {
	...
	for (cluster c : static_cast<CPlanarityMaster*>(master_)->getClusterGraph()->clusters) {
		...
		if (isConnected(*c_support)) {
			...
			if (mincutV < 1.0 - master()->eps() - minViolate) {
				...
				cout << "\t" << ssg.numberOfNodes() << " " << ssg.numberOfEdges() << static_cast<CPlanarityMaster*>(master())->searchSpaceGraph()->numberOfEdges() << "\n";
	...
	for (int i = 0; i < static_cast<CPlanarityMaster*>(master_)->getNKuratowskiSupportGraphs(); ++i) {
		...
		while (static_cast<CPlanarityMaster*>(master_)->getKIterations() >= iteration) {
			...
			if (leftHandSide > ks.varnum - (1 - master()->eps() - minViolate)) {
				...
				{
					cout << "Edge between " << e->source() << "-" << e->target() << " in supportgraph\n";
				}
				NodeArray<int> potDeg(support, 0); //number of potential additionial edges
				for (int i = 0; i < nVar(); ++i) {
					node v = static_cast<EdgeVar*>(variable(i))->sourceNode();
					node w = static_cast<EdgeVar*>(variable(i))->targetNode();
