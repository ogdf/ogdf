...
#include <ogdf/basic/basic.h>                      // for OGDF_ASSERT, min, string
...
#include <ogdf/orthogonal/OrthoRep.h>              // for OrthoDir, OrthoRep, BendString
#include <ogdf/orthogonal/edge_router/NodeInfo.h>  // for NodeInfo
...
//constructor
EdgeRouter::EdgeRouter(PlanRep& pru, OrthoRep& H, GridLayoutMapped& L, CombinatorialEmbedding& E,
		RoutingChannel<int>& rou, MinimumEdgeDistances<int>& mid, NodeArray<int>& nodewidth,
...
void EdgeRouter::init(PlanRep& pru, RoutingChannel<int>& rou, bool align) {
...
void EdgeRouter::call(PlanRep& pru, OrthoRep& H, GridLayoutMapped& L, CombinatorialEmbedding& E,
		RoutingChannel<int>& rou, MinimumEdgeDistances<int>& mid, NodeArray<int>& nodewidth,
		NodeArray<int>& nodeheight, bool align) {
	string msg;
	...
	if (m_minDelta) {
		for (node v : pru.nodes) {
			if ((pru.expandAdj(v) != nullptr)
				...
				if (vdeg != 0) {
					Math::updateMin(mysep, int(floor((double)perimeter / vdeg)));
				}
			}
		}
	}
	...
	auto createComputeValuesFunc = [&](bool leftRight) {
		...
		return [&, dirA, dirB](const OrthoDir dir, NodeInfo& inf) {
			...
			for (edge inedge : side_in_edges) {
				...
				alow[outEntry(inf, dir, pos_e - 1)] =
						pos[inedge->target()] - sepsA - inf.eps(dir, dirA);
				upp[inedge] = pos[inedge->target()] + sepsB + inf.eps(dir, dirB);
				aupp[outEntry(inf, dir, pos_e - 1)] =
						pos[inedge->target()] + sepsB + inf.eps(dir, dirB);
	...
	for (node l_v : pru.nodes) {
		//check if replaced
		if ((pru.expandAdj(l_v) != nullptr)
				&& (pru.typeOf(l_v) != Graph::NodeType::generalizationMerger)) {
	...
	for (node v : pru.nodes) {
		// pru.typeOf(v) == Graph::highDegreeExpander) ) // expanded high degree
		if ((pru.expandAdj(v) != nullptr)
				&& (pru.typeOf(v) != Graph::NodeType::generalizationMerger)) {
	...
	for (node v : pru.nodes) {
		if (pru.expandAdj(v) != nullptr && pru.typeOf(v) != Graph::NodeType::generalizationMerger
...
//and generalizations for horizontal edges and set bend type accordingly
{
	...
	int ybase = 0;
	int gen_y = infos[v].coord(OrthoDir::West) + int(floor((double)(infos[v].node_ysize()) / 2));
	//in the middle
	//y coordinates
	...
	{
		int pos = infos[v].gen_pos(OrthoDir::North)
				- 1; //compare edge position to generalization position
		if (pos > -1) {
			l_it = infos[v].inList(OrthoDir::North).get(pos); //muss unter gen sein, -2???
		} else {
			l_it = nullptr;
			pos = 0;
		}
		...
		while (l_it.valid()
				&& (pos * infos[v].delta(OrthoDir::North, OrthoDir::West)
								+ infos[v].eps(OrthoDir::North, OrthoDir::West)
						<= (cp_y(outEntry(infos[v], OrthoDir::North, pos))
								- infos[v].coord(OrthoDir::West))))
		//bendfree edges cageCoord??!!!
		{
			m_agp_y[outEntry(infos[v], OrthoDir::North, pos)] =
					cp_y(outEntry(infos[v], OrthoDir::North, pos));
			...
			ybase = cp_y(outEntry(infos[v], OrthoDir::North, pos))
					- infos[v].delta(OrthoDir::North, OrthoDir::West);
			--l_it;
			--pos;
		...
		while (l_it.valid()) {
			...
			m_agp_y[adj] = inf.coord(OrthoDir::West) + inf.eps(OrthoDir::North, OrthoDir::West)
					+ pos * inf.delta(OrthoDir::North, OrthoDir::West);

			updateOneBend(cp_y(adj) >= inf.coord(OrthoDir::West) - m_sep, adj, v, OrthoDir::North,
					true, BendType::ProbB1L, BendType::ProbB2L);
		...
		m_abends[outEntry(infos[v], OrthoDir::North, infos[v].gen_pos(OrthoDir::North))] =
				BendType::BendFree;

		if (lastcheck) {
			infos[v].set_l_lower(ybase);
		}
		...
		while (l_it.valid()
				&& ((infos[v].inList(OrthoDir::North).size() - 1 - pos)
										* infos[v].delta(OrthoDir::North, OrthoDir::East)
								+ infos[v].eps(OrthoDir::North, OrthoDir::East)
						<= (infos[v].coord(OrthoDir::East)
								- cp_y(outEntry(infos[v], OrthoDir::North, pos))))) {
			m_abends[outEntry(infos[v], OrthoDir::North, pos)] = BendType::BendFree;
			ybase = cp_y(outEntry(infos[v], OrthoDir::North,
					pos)); //+= infos[v].delta(OrthoDir::North, OrthoDir::East);
		...
		while (l_it.valid()) {
			...
			ybase = inf.l_upper_unbend()
					+ (pos + 1 + bendnum - inf.inList(OrthoDir::North).size())
							* inf.delta(OrthoDir::North, OrthoDir::East);

			updateOneBend(m_acp_y[adj] < inf.coord(OrthoDir::East) + m_sep, adj, v, OrthoDir::North,
					false, BendType::ProbB1R, BendType::ProbB2R);
	...
	//NO LEFT GENERATOR
		...
		while (l_it.valid()
			...
			m_agp_y[outEntry(infos[v], OrthoDir::North, pos)] =
					cp_y(outEntry(infos[v], OrthoDir::North, pos));
			//m_acp_y[outEntry(infos[v], OrthoDir::North, pos)];
			++l_it;
			++pos;
		}
	...
	if (infos[v].has_gen(OrthoDir::South)) {
		//left copy
		int pos = infos[v].gen_pos(OrthoDir::South)
				- 1; //compare edge position to generalization position
		if (pos > -1) {
		...
		bool lastcheck = true;
		while (l_it.valid()
				&& (pos * infos[v].delta(OrthoDir::South, OrthoDir::West)
								+ infos[v].eps(OrthoDir::South, OrthoDir::West)
						<= (cp_y(outEntry(infos[v], OrthoDir::South, pos))
								- infos[v].coord(OrthoDir::West)))) {
			m_agp_y[outEntry(infos[v], OrthoDir::South, pos)] =
					cp_y(outEntry(infos[v], OrthoDir::South, pos));
			...
			ybase = cp_y(outEntry(infos[v], OrthoDir::South, pos))
					- infos[v].delta(OrthoDir::South, OrthoDir::West);
			--l_it;
			--pos;
		...
		m_abends[outEntry(infos[v], OrthoDir::South, infos[v].gen_pos(OrthoDir::South))] =
				BendType::BendFree;
		if (lastcheck) {
			infos[v].set_r_lower(
					m_agp_y[outEntry(infos[v], OrthoDir::South, infos[v].gen_pos(OrthoDir::South))]);
		}
		...
		while (l_it.valid()
				&& ((infos[v].inList(OrthoDir::South).size() - 1 - pos)
										* infos[v].delta(OrthoDir::South, OrthoDir::East)
								+ infos[v].eps(OrthoDir::South, OrthoDir::East)
						<= (infos[v].coord(OrthoDir::East)
								- cp_y(outEntry(infos[v], OrthoDir::South, pos))))) {
			m_abends[outEntry(infos[v], OrthoDir::South, pos)] = BendType::BendFree;
			ybase = cp_y(outEntry(infos[v], OrthoDir::South,
					pos)); //+= infos[v].delta(OrthoDir::North, OrthoDir::East);
		...
		while (l_it.valid()) {
			...
			// check for single/2 bend
			updateOneBend(cp_y(adj) <= inf.coord(OrthoDir::East) + m_sep, adj, v, OrthoDir::South,
					true, BendType::ProbB1L, BendType::ProbB2L);
...
void EdgeRouter::compute_gen_glue_points_x(node v) {
	...
	{
		int pos = infos[v].gen_pos(OrthoDir::East)
				- 1; //compare edge position to generalization position
		if (pos > -1) {
		...
		while (l_it.valid()
			...
			xbase = cp_x(outEntry(infos[v], OrthoDir::East, pos))
					- infos[v].delta(OrthoDir::East, OrthoDir::North);
			--l_it;
			--pos;
		...
		m_abends[outEntry(infos[v], OrthoDir::East, infos[v].gen_pos(OrthoDir::East))] =
				BendType::BendFree;
		infos[v].num_bend_free_increment(OrthoDir::East);
		if (!check) {
			infos[v].set_t_left(
					m_agp_x[outEntry(infos[v], OrthoDir::East, infos[v].gen_pos(OrthoDir::East))]);
		}
		infos[v].set_t_right(
				m_agp_x[outEntry(infos[v], OrthoDir::East, infos[v].gen_pos(OrthoDir::East))]);
		...
		while (l_it.valid()
				&& ((infos[v].inList(OrthoDir::East).size() - 1 - pos)
										* infos[v].delta(OrthoDir::East, OrthoDir::South)
								+ infos[v].eps(OrthoDir::East, OrthoDir::South)
						<= (infos[v].coord(OrthoDir::South)
								- cp_x(outEntry(infos[v], OrthoDir::East, pos))))) {
			m_abends[outEntry(infos[v], OrthoDir::East, pos)] = BendType::BendFree;
			xbase = cp_x(outEntry(infos[v], OrthoDir::East,
					pos)); //+= infos[v].delta(OrthoDir::East, OrthoDir::South);
		...
		while (l_it.valid()) {
			...
			//check for single/2 bend
			updateOneBend(m_acp_x[adj] < inf.coord(OrthoDir::South) + m_sep, adj, v, OrthoDir::East,
					false, BendType::ProbB1R, BendType::ProbB2R);
		...
		while (l_it.valid()
			...
			m_abends[outEntry(infos[v], OrthoDir::East, pos)] = BendType::BendFree;
			infos[v].num_bend_free_increment(OrthoDir::East);
			lastunbend = m_agp_x[outEntry(infos[v], OrthoDir::East, pos)] =
					cp_x(outEntry(infos[v], OrthoDir::East, pos));
	...
	if (infos[v].has_gen(OrthoDir::West)) {
		//left copy
		int pos = infos[v].gen_pos(OrthoDir::West)
				- 1; //compare edge position to generalization position
		if (pos > -1) {
		...
		bool firstcheck = true;
		while (l_it.valid()
				&& (pos * infos[v].delta(OrthoDir::West, OrthoDir::North)
								+ infos[v].eps(OrthoDir::West, OrthoDir::North)
						<= (cp_x(outEntry(infos[v], OrthoDir::West, pos))
								- infos[v].coord(OrthoDir::North)))) {
			m_agp_x[outEntry(infos[v], OrthoDir::West, pos)] =
					cp_x(outEntry(infos[v], OrthoDir::West, pos));
			m_abends[outEntry(infos[v], OrthoDir::West, pos)] = BendType::BendFree;
			xbase = cp_x(outEntry(infos[v], OrthoDir::West, pos))
					- infos[v].delta(OrthoDir::West, OrthoDir::North);
			if (firstcheck) {
				firstcheck = false;
		...
		m_abends[outEntry(infos[v], OrthoDir::West, infos[v].gen_pos(OrthoDir::West))] =
				BendType::BendFree;
		infos[v].num_bend_free_increment(OrthoDir::West);
		if (firstcheck) {
			firstcheck = false;
			infos[v].set_b_right(
					m_agp_x[outEntry(infos[v], OrthoDir::West, infos[v].gen_pos(OrthoDir::West))]);
		}
		infos[v].set_b_left(
				m_agp_x[outEntry(infos[v], OrthoDir::West, infos[v].gen_pos(OrthoDir::West))]);
		...
		while (l_it.valid()
				&& ((infos[v].inList(OrthoDir::West).size() - 1 - pos)
										* infos[v].delta(OrthoDir::West, OrthoDir::South)
								+ infos[v].eps(OrthoDir::West, OrthoDir::South)
						<= (infos[v].coord(OrthoDir::South)
								- cp_x(outEntry(infos[v], OrthoDir::West, pos))))) {
			m_abends[outEntry(infos[v], OrthoDir::West, pos)] = BendType::BendFree;
			xbase = cp_x(outEntry(infos[v], OrthoDir::West,
					pos)); //+= infos[v].delta(OrthoDir::North, OrthoDir::East);
		...
		while (l_it.valid()) {
			...
			//check for single/2 bend
			updateOneBend(m_acp_x[adj] <= inf.coord(OrthoDir::South) + m_sep, adj, v,
					OrthoDir::West, true, BendType::ProbB1L, BendType::ProbB2L);
		...
		while (l_it.valid()
			...
			m_abends[outEntry(infos[v], OrthoDir::West, pos)] = BendType::BendFree;
			infos[v].num_bend_free_increment(OrthoDir::West);
			lastunbend = m_agp_x[outEntry(infos[v], OrthoDir::West, pos)] =
					cp_x(outEntry(infos[v], OrthoDir::West, pos));
		...
		if (firstunbend != m_init) {
			...
			while (ll_it.valid()
					&& (infos[v].coord(OrthoDir::North)
							> (cp_x(outEntry(infos[v], OrthoDir::West, llpos))
									- llpos * infos[v].delta(OrthoDir::West, OrthoDir::North)
									- infos[v].eps(OrthoDir::West, OrthoDir::North)))) {
...
void EdgeRouter::compute_glue_points_y(node v) {
	...
	while (l_it.valid()
		...
		m_abends[outEntry(infos[v], OrthoDir::North, pos)] = BendType::BendFree;
		infos[v].num_bend_free_increment(OrthoDir::North);
		lastunbend = m_agp_y[outEntry(infos[v], OrthoDir::North, pos)] =
				cp_y(outEntry(infos[v], OrthoDir::North, pos));
		if (firstcheck) {
	...
	if (firstunbend != m_init) {
		...
		int llpos = 0;
		while (ll_it.valid()
				&& (infos[v].coord(OrthoDir::West) > (cp_y(outEntry(infos[v], OrthoDir::North, llpos))
							- llpos * infos[v].delta(OrthoDir::North, OrthoDir::West)
							- infos[v].eps(OrthoDir::North, OrthoDir::West)))) {
	...
	while (l_it.valid()
		...
		m_abends[outEntry(infos[v], OrthoDir::South, pos)] = BendType::BendFree;
		infos[v].num_bend_free_increment(OrthoDir::South);
		lastunbend = m_agp_y[outEntry(infos[v], OrthoDir::South, pos)] =
				cp_y(outEntry(infos[v], OrthoDir::South, pos));
		if (firstcheck) {
			firstcheck = false;
			infos[v].set_r_lower(m_agp_y[outEntry(infos[v], OrthoDir::South, pos)]);
	...
	if (firstunbend != m_init) {
		...
		int llpos = 0;
		while (ll_it.valid()
				&& (infos[v].coord(OrthoDir::West) > (cp_y(outEntry(infos[v], OrthoDir::South, llpos))
							- llpos * infos[v].delta(OrthoDir::South, OrthoDir::West)
							- infos[v].eps(OrthoDir::South, OrthoDir::West)))) {
...
//maybe: use earlier classification of edges: bendfree?
{
	...
	if (firstunbend != m_init) {
		...
		int llpos = 0;
		while (ll_it.valid()
				&& (infos[v].coord(OrthoDir::North) > (cp_x(outEntry(infos[v], OrthoDir::East, llpos))
							- llpos * infos[v].delta(OrthoDir::East, OrthoDir::North)
							- infos[v].eps(OrthoDir::East, OrthoDir::North)))) {
			m_agp_x[outEntry(infos[v], OrthoDir::East, llpos)] = firstunbend
					- (numbends - llpos) * infos[v].delta(OrthoDir::East, OrthoDir::North);
	...
	while (l_it.valid()) //&&???!!!
	{
		if (cp_x(outEntry(infos[v], OrthoDir::East, pos)) <= infos[v].coord(OrthoDir::South) + m_sep) {
			if (cp_x(outEntry(infos[v], OrthoDir::East, pos)) > infos[v].coord(OrthoDir::South)
							- infos[v].eps(OrthoDir::East, OrthoDir::South)) {
		...
		if (lastunbend != m_init) {
			m_agp_x[outEntry(infos[v], OrthoDir::East, pos)] =
					lastunbend + infos[v].delta(OrthoDir::East, OrthoDir::South);
			lastunbend = lastunbend + infos[v].delta(OrthoDir::East, OrthoDir::South);
		} else {
			m_agp_x[outEntry(infos[v], OrthoDir::East, pos)] = infos[v].coord(OrthoDir::South)
					- infos[v].eps(OrthoDir::East, OrthoDir::South)
					- (infos[v].inList(OrthoDir::East).size() - 1 - pos)
							* infos[v].delta(OrthoDir::East, OrthoDir::South);
		}
	...
	if (firstunbend != m_init) {
		...
		int llpos = 0;
		while (ll_it.valid()
				&& (infos[v].coord(OrthoDir::North) > (cp_x(outEntry(infos[v], OrthoDir::West, llpos))
							- llpos * infos[v].delta(OrthoDir::West, OrthoDir::North)
							- infos[v].eps(OrthoDir::West, OrthoDir::North)))) {
...
void EdgeRouter::compute_place(node v, NodeInfo& inf /*, int l_sep, int l_overh*/) {
	...
	if (lhorz_size && (!horizontal_merger)) {
		...
		uppe_l = li_l.valid() ? auppe[outEntry(inf, OrthoDir::North, 0)]
							  : std::numeric_limits<int>::max(); // only both 10000.0 if both empty!!!
		uppe_r = li_r.valid() ? auppe[outEntry(inf, OrthoDir::South, 0)]
							  : std::numeric_limits<int>::max(); // never in for - loop
		lowe_l = li_ll.valid()
				? alowe[outEntry(inf, OrthoDir::North, 0)]
				: std::numeric_limits<int>::max(); // only both 10000.0 if both empty!!!
		lowe_r = li_lr.valid() ? alowe[outEntry(inf, OrthoDir::South, 0)]
							   : std::numeric_limits<int>::max(); // never in for - loop
		...
		for (int k = 0; k < lhorz_size; k++) {
			...
			if (uppe_l <= uppe_r) //favour left edges, maybe we should prefer them
			{
				...
				++li_l;
				++lcount;
				uppe_l = lcount < inf.inList(OrthoDir::North).size()
						? auppe[outEntry(inf, OrthoDir::North, lcount)]
				...
				++li_r;
				++rcount;
				uppe_r = rcount < inf.inList(OrthoDir::South).size()
						? auppe[outEntry(inf, OrthoDir::South, rcount)]
						: std::numeric_limits<int>::max();
			}
			horz_entry[e] = l_horz.pushBack(e); //fill edge in L

			value_entry[e] =
					edgevalue.pushBack(m_prup->expandedNode(nextNeighbour) ? l_directvalue : 1);
			...
			if (lowe_l <= lowe_r) //favour left edges, maybe we should prefer them
			{
				...
				++li_ll;
				++llcount;
				lowe_l = llcount < inf.inList(OrthoDir::North).size()
						? alowe[outEntry(inf, OrthoDir::North, llcount)]
						: std::numeric_limits<int>::max();
			} else {
				e = *li_lr;
				++li_lr;
				...
				lowe_r = rlcount < inf.inList(OrthoDir::South).size()
						? alowe[outEntry(inf, OrthoDir::South, rlcount)]
						: std::numeric_limits<int>::max();
			}
	...
	if (lvert_size && !vertical_merger) {
		...
		righte_t = li_t.valid()
				? arighte[outEntry(inf, OrthoDir::East, 0)]
				: std::numeric_limits<int>::max(); // only both 10000.0 if both empty!!!
		righte_b = li_b.valid() ? arighte[outEntry(inf, OrthoDir::West, 0)]
								: std::numeric_limits<int>::max(); // never in for - loop
		lefte_t = li_lt.valid()
				? alefte[outEntry(inf, OrthoDir::East, 0)]
				: std::numeric_limits<int>::max(); // only both 10000.0 if both empty!!!
		lefte_b = li_lb.valid() ? alefte[outEntry(inf, OrthoDir::West, 0)]
								: std::numeric_limits<int>::max(); // never in for - loop
		...
		for (int k = 0; k < lvert_size; k++) {
			...
			if (li_t.valid() || li_b.valid()) {
				...
				if (righte_t <= righte_b) //favour top edges, maybe we should prefer them
				{
					...
					righte_t = tcount < inf.inList(OrthoDir::East).size()
							? arighte[outEntry(inf, OrthoDir::East, tcount)]
							: std::numeric_limits<int>::max();
				} else {
					if (li_b.valid()) {
						e = *li_b;
					...
					righte_b = bcount < inf.inList(OrthoDir::West).size()
							? arighte[outEntry(inf, OrthoDir::West, bcount)]
							: std::numeric_limits<int>::max();
				}
			...
			if (lefte_t <= lefte_b) //favour top edges, maybe we should prefer them
			{
				...
				lefte_t = tlcount < inf.inList(OrthoDir::East).size()
						? alefte[outEntry(inf, OrthoDir::East, tlcount)]
						: std::numeric_limits<int>::max();
			} else {
				e = *li_lb;
				++li_lb;
				...
				lefte_b = blcount < inf.inList(OrthoDir::West).size()
						? alefte[outEntry(inf, OrthoDir::West, blcount)]
						: std::numeric_limits<int>::max();
			}
			OGDF_ASSERT(e);
			l_vertl.pushBack(e);
	...
	if (!l_horz.empty()) {
		...
		if (l_horz.size() == 1) {
			...
			if (at_left[*(l_horz.begin())]) {
				...
				bestvalue = alowe[outEntry(inf, OrthoDir::South, 0)];
			}
		} else {
			...
			for (i = 1; i < stop + 1; i++) //in der Regel laenger als noetig??
			{
				...
				while (p.valid()) {
					...
					if (uppe[*p] <= (lowe[l_horzl.front()] + boxy))
					//+machineeps)) //||(p == horz_entry[l_horzl.front()]))
				...
				if (num_unbend > best_unbend) {
					...
					bestvalue = at_left[l_horzl.front()]
							? alowe[outEntry(inf, OrthoDir::North, leftcount)]
							: alowe[outEntry(inf, OrthoDir::South, rightcount)];
				}

				if (at_left[l_horzl.front()]) {
					leftcount++;
				} else {
					rightcount++;
				}

				if (p == horz_entry[l_horzl.front()]) {
					++p;
				}
				if (valp == value_entry[l_horzl.front()]) {
					++valp;
				}
		...
		m_newy[v] = min((inf.cageCoord(OrthoDir::East) - inf.node_ysize() - inf.rc(OrthoDir::East)),
				bestvalue);
		...
		if (horizontal_merger) {
			...
			if (inf.has_gen(OrthoDir::North)) {
				e = *(inf.inList(OrthoDir::North).get(inf.gen_pos(OrthoDir::North)));
			} else {
				e = *(inf.inList(OrthoDir::South).get(inf.gen_pos(OrthoDir::South))); // XXX: check e
			}
			...
			if (vertical_merger) {
				...
				if (wg) {
					if (eg) {
						if (inf.is_in_edge(OrthoDir::West, inf.gen_pos(OrthoDir::West)))
						//position to OrthoDir::East
						{
							mynewy = inf.cageCoord(OrthoDir::East) - inf.rc(OrthoDir::East)
									- inf.node_ysize();
						} else {
	...
	if (!l_vert.empty()) {
		...
		if (l_vert.size() == 1) {
			...
			if (at_top[*(l_vert.begin())]) {
				...
				bestvalue = alefte[outEntry(inf, OrthoDir::West, 0)];
			}
		} else {
			//ALGORITHM 1
			...
			for (i = 1; i < stop + 1; i++) {
...
#endif
					//p's entry needs no bend => increase num_unbend (edges)
					if (righte[*p] <= lefte[l_vertl.front()] + boxx + machineeps) {
						//assert first edge is edge i?
						num_unbend++;
						++p;
					} else {
						break;
				...
				if (num_unbend > best_unbend) {
					...
					bestvalue = at_top[l_vertl.front()]
							? alefte[outEntry(inf, OrthoDir::East, topcount)]
							: alefte[outEntry(inf, OrthoDir::West, lowcount)];
				}
				if (at_top[l_vertl.front()]) {
					topcount++;
				} else {
					lowcount++;
				}
				...
				if (num_unbend) {
					num_unbend--; //the next index will bend current start edge
				}
			}
		}
		...
		//- (at_left[bestedge] ?
		// inf.eps(OrthoDir::North, OrthoDir::West) : inf.eps(OrthoDir::South, OrthoDir::West))));
		// maybe top / down epsilon
	} else {
		if (vertical_merger) {
			//position is fixed //North was OrthoDir::North
			edge e;
			//note that get starts indexing with zero, whereas gen_pos starts with one
			if (inf.has_gen(OrthoDir::East)) {
				e = *(inf.inList(OrthoDir::East).get(inf.gen_pos(OrthoDir::East)));
			} else {
				e = *(inf.inList(OrthoDir::West).get(inf.gen_pos(OrthoDir::West))); // XXX: check e
			}
			...
			if (horizontal_merger) {
				...
				if (sg) {
					if (ng) {
						// position to OrthoDir::North
						if (inf.is_in_edge(OrthoDir::South, inf.gen_pos(OrthoDir::South))) {
							mynewx = inf.cageCoord(OrthoDir::North) + inf.rc(OrthoDir::North);
						} else {
							mynewx = inf.cageCoord(OrthoDir::South) - inf.rc(OrthoDir::South)
									- inf.node_xsize();
						}
					} else {
						mynewx = inf.cageCoord(OrthoDir::South) - inf.rc(OrthoDir::South)
				...
				m_newx[v] =
						inf.cageCoord(OrthoDir::South) - inf.rc(OrthoDir::South) - inf.node_xsize();
				inf.set_coord(OrthoDir::North, m_newx[v]);
				inf.set_coord(OrthoDir::South, m_newx[v] + inf.node_xsize());
			}
		}
	...
	if (m_mergerSon[v]) {
		if (vertical_merger) //change vertical position
		{
			...
			if (m_mergeDir[v] == OrthoDir::North) {
				m_newy[v] =
						(inf.cageCoord(OrthoDir::East) - inf.node_ysize() - inf.rc(OrthoDir::East));
			} else {
	...
	if (horizontal_merger) {
		...
		compute_glue_points_y(v);
	}
	if (vertical_merger) {
		compute_gen_glue_points_x(v);
	} else {
		compute_glue_points_x(v);
	}
	set_corners(v);
...
void EdgeRouter::place(node l_v) {
	...
	int leftofs = infos[l_v].num_bend_free(OrthoDir::North)
			? 0
			: infos[l_v].delta(OrthoDir::North, OrthoDir::West)
					* infos[l_v].flips(OrthoDir::West, OrthoDir::North);
	int rightofs = infos[l_v].num_bend_free(OrthoDir::North)
			? 0
			: infos[l_v].delta(OrthoDir::North, OrthoDir::East)
					* infos[l_v].flips(OrthoDir::East, OrthoDir::North);
	...
	while (it.valid()) {
		...
		if (!((inedge && (v == e->target())) || ((v == e->source()) && !inedge))) {
			edge run = e;
			if (inedge) {
		...
		if (abendType(ae) != BendType::BendFree) {
			...
			case BendType::Bend1Left: //rerouted single bend
				...
				if ((!acorn) && (ipos == infos[l_v].flips(OrthoDir::North, OrthoDir::West) - 1)) {
					...
					fix_position(newe2->source(), infos[l_v].coord(OrthoDir::North),
							infos[l_v].coord(OrthoDir::West));
			...
			case BendType::Bend1Right:
				...
				xtacy = infos[l_v].coord(OrthoDir::North)
						+ (ipos + infos[l_v].flips(OrthoDir::North, OrthoDir::East)
								  - infos[l_v].inList(OrthoDir::North).size())
								* infos[l_v].delta(OrthoDir::East, OrthoDir::North)
						+ infos[l_v].eps(OrthoDir::East, OrthoDir::North);
				if (inedge) {
					newe = addRightBend(e);
				} else {
					newe = addLeftBend(e);
			...
			case BendType::Bend2Right:
				...
				xtacy = infos[l_v].cageCoord(OrthoDir::North)
						+ (1 + ipos + infos[l_v].num_bend_edges(OrthoDir::North, OrthoDir::East)
								  - infos[l_v].inList(OrthoDir::North).size())
								* m_sep;
				//* infos[l_v].delta(OrthoDir::North, OrthoDir::East);
				newe = addRightBend(e);
	...
	//EAST SIDE (bottom)
	leftofs = infos[l_v].num_bend_free(OrthoDir::East)
			? 0
			: infos[l_v].delta(OrthoDir::East, OrthoDir::North)
					* infos[l_v].flips(OrthoDir::North, OrthoDir::East);
	//temp
	//leftofs = infos[l_v].delta(OrthoDir::East, OrthoDir::North)*infos[l_v].flips(OrthoDir::North, OrthoDir::East);
	//temp end
	rightofs = infos[l_v].num_bend_free(OrthoDir::East)
			? 0
			: infos[l_v].delta(OrthoDir::East, OrthoDir::South)
					* infos[l_v].flips(OrthoDir::South, OrthoDir::East);
	//temp
	...
	while (it.valid()) {
		...
		if (abendType(ae) != BendType::BendFree) {
			...
			case BendType::Bend1Left:
				...
				if (inedge) {
					newe = addLeftBend(e); //abhaengig von inedge, ??
				} else {
					newe = addRightBend(e);
				}
				newbend = newe->source();
				...
				ypsiqueen = infos[l_v].coord(OrthoDir::East)
						- (infos[l_v].flips(OrthoDir::East, OrthoDir::North) - ipos - 1)
								* infos[l_v].delta(OrthoDir::North, OrthoDir::East)
						- infos[l_v].eps(OrthoDir::North, OrthoDir::East);
			...
			case BendType::Bend1Right:
				...
				if (!corn) {
					...
					fix_position(newe2->source(), infos[l_v].coord(OrthoDir::South),
							infos[l_v].coord(OrthoDir::East));
					m_prup->setExpandedNode(newe2->source(), l_v);
				}
				ypsiqueen = infos[l_v].coord(OrthoDir::East)
						- (ipos + infos[l_v].flips(OrthoDir::East, OrthoDir::South)
								  - infos[l_v].inList(OrthoDir::East).size())
								* infos[l_v].delta(OrthoDir::South, OrthoDir::East)
						- infos[l_v].eps(OrthoDir::South, OrthoDir::East);
				//delete corner after last rerouting
			...
			case BendType::Bend2Right:
				...
				ypsiqueen = infos[l_v].cageCoord(OrthoDir::East)
						- (ipos - infos[l_v].inList(OrthoDir::East).size()
								  + infos[l_v].num_bend_edges(OrthoDir::East, OrthoDir::South))
								* m_sep;
	...
	//SOUTH SIDE
	leftofs = infos[l_v].num_bend_free(OrthoDir::South)
			? 0
			: infos[l_v].delta(OrthoDir::South, OrthoDir::East)
					* infos[l_v].flips(OrthoDir::East, OrthoDir::South);
	rightofs = infos[l_v].num_bend_free(OrthoDir::South)
			? 0
			: infos[l_v].delta(OrthoDir::South, OrthoDir::West)
					* infos[l_v].flips(OrthoDir::West, OrthoDir::South);
		...
		if (abendType(ae) != BendType::BendFree) {
			...
			case BendType::Bend1Left:
				...
				xtacy = infos[l_v].coord(OrthoDir::South)
						- infos[l_v].delta(OrthoDir::East, OrthoDir::South)
								* (infos[l_v].flips(OrthoDir::South, OrthoDir::East) + ipos
										- infos[l_v].inList(OrthoDir::South).size())
			...
			case BendType::Bend2Left:
				...
				newe = addLeftBend(e);
					...
					if (inedge) {
						fix_position(e->target(), m_agp_x[ae], gy);
					} else {
						fix_position(e->source(), m_agp_x[ae], gy);
					}
#endif
				fix_position(newe->source(), xtacy, inedge ? cp_y(ae) : (gp_y(ae) - leftofs));
				newe = addRightBend(newe);
			...
			case BendType::Bend1Right:
				...
				xtacy = infos[l_v].coord(OrthoDir::South)
						- (infos[l_v].flips(OrthoDir::South, OrthoDir::West) - ipos - 1)
								* infos[l_v].delta(OrthoDir::West, OrthoDir::South)
						- infos[l_v].eps(OrthoDir::West, OrthoDir::South);
				fix_position(newbend, xtacy, cp_y(ae));
				fix_position(newglue, xtacy, infos[l_v].coord(OrthoDir::West));
				break; //rerouted single bend
			case BendType::ProbB1R:
	...
	//WEST SIDE
	leftofs = infos[l_v].num_bend_free(OrthoDir::West)
			? 0
			: infos[l_v].delta(OrthoDir::West, OrthoDir::South)
					* infos[l_v].flips(OrthoDir::South, OrthoDir::West);
	rightofs = infos[l_v].num_bend_free(OrthoDir::West)
			? 0
			: infos[l_v].delta(OrthoDir::West, OrthoDir::North)
					* infos[l_v].flips(OrthoDir::North, OrthoDir::West);
		...
		if (abendType(ae) != BendType::BendFree) {
			...
			case BendType::Bend1Left:
				...
				ypsiqueen = infos[l_v].coord(OrthoDir::West)
						+ (infos[l_v].flips(OrthoDir::West, OrthoDir::South) + ipos
								  - infos[l_v].inList(OrthoDir::West).size())
								* infos[l_v].delta(OrthoDir::South, OrthoDir::West)
			...
			case BendType::Bend1Right:
				...
				ypsiqueen = infos[l_v].coord(OrthoDir::West)
						+ (infos[l_v].flips(OrthoDir::West, OrthoDir::North) - ipos - 1)
								* infos[l_v].delta(OrthoDir::North, OrthoDir::West)
						+ infos[l_v].eps(OrthoDir::North, OrthoDir::West);
				fix_position(newbend, cp_x(ae), ypsiqueen);
				fix_position(newglue, infos[l_v].coord(OrthoDir::North), ypsiqueen);
				break; //rerouted single bend
			case BendType::ProbB1R:
...
void EdgeRouter::compute_routing(node v) //reroute(face f)
{
	...
	//top-left
	int flip_tl, flip_lt;
	if (compute_move(OrthoDir::East, OrthoDir::North, flip_tl, v)
			< compute_move(OrthoDir::North, OrthoDir::East, flip_lt, v)) {
		flip_tl = 0;
	} else {
		flip_lt = 0;
	}
	//left-bottom
	int flip_lb, flip_bl;
	if (compute_move(OrthoDir::North, OrthoDir::West, flip_lb, v)
			< compute_move(OrthoDir::West, OrthoDir::North, flip_bl, v)) {
		flip_lb = 0;
	} else {
		flip_bl = 0;
	}
	//top-right
	int flip_tr, flip_rt;
	if (compute_move(OrthoDir::East, OrthoDir::South, flip_tr, v)
			< compute_move(OrthoDir::South, OrthoDir::East, flip_rt, v)) {
		flip_tr = 0;
	} else {
		flip_rt = 0;
	}
	//bottom-right
	int flip_br, flip_rb;
	if (compute_move(OrthoDir::West, OrthoDir::South, flip_br, v)
			< compute_move(OrthoDir::South, OrthoDir::West, flip_rb, v)) {
		flip_br = 0;
	} else {
		flip_rb = 0;
	}
	...
	for (flipedges = 0; flipedges < flip_lt; flipedges++) {
		m_abends[outEntry(infos[v], OrthoDir::North,
				infos[v].inList(OrthoDir::North).size() - 1 - flipedges)] = BendType::Bend1Right;
	...
	if (flip_lt) //we flipped and may save bends, if may be not necessary cause fliplt parameter in betamove
	{
		...
		for (newbf = 0; newbf < newbendfree; newbf++) {
			m_abends[outEntry(infos[v], OrthoDir::North,
					infos[v].inList(OrthoDir::North).size() - 1 - flip_lt - newbf)] =
					BendType::BendFree;
			m_agp_y[outEntry(infos[v], OrthoDir::North,
					infos[v].inList(OrthoDir::North).size() - 1 - flip_lt - newbf)] =
					cp_y(outEntry(infos[v], OrthoDir::North,
							infos[v].inList(OrthoDir::North).size() - 1 - flip_lt - newbf));
		}
	}
	...
	for (flipedges = 0; flipedges < flip_rt; flipedges++) {
		...
		m_abends[outEntry(infos[v], OrthoDir::South,
				infos[v].inList(OrthoDir::South).size() - 1 - flipedges)] = BendType::Bend1Left;
	}

	newbendfree = beta_move(OrthoDir::South, OrthoDir::East, flip_rt, v);
	for (newbf = 0; newbf < newbendfree; newbf++) {
		m_abends[outEntry(infos[v], OrthoDir::South,
				infos[v].inList(OrthoDir::South).size() - 1 - flip_rt - newbf)] = BendType::BendFree;
		m_agp_y[outEntry(infos[v], OrthoDir::South,
				infos[v].inList(OrthoDir::South).size() - 1 - flip_rt - newbf)] =
				cp_y(outEntry(infos[v], OrthoDir::South,
						infos[v].inList(OrthoDir::South).size() - 1 - flip_rt - newbf));
	}
	...
	for (flipedges = 0; flipedges < flip_tr; flipedges++) {
		...
		{
			m_abends[outEntry(infos[v], OrthoDir::East,
					infos[v].inList(OrthoDir::East).size() - 1 - flipedges)] = BendType::Bend1Right;
	...
	for (newbf = 0; newbf < newbendfree; newbf++) {
		m_abends[outEntry(infos[v], OrthoDir::East,
				infos[v].inList(OrthoDir::East).size() - 1 - flip_tr - newbf)] = BendType::BendFree;
		m_agp_x[outEntry(infos[v], OrthoDir::East,
				infos[v].inList(OrthoDir::East).size() - 1 - flip_tr - newbf)] =
				cp_x(outEntry(infos[v], OrthoDir::East,
						infos[v].inList(OrthoDir::East).size() - 1 - flip_tr - newbf));
	}

	for (flipedges = 0; flipedges < flip_br; flipedges++) {
		m_abends[outEntry(infos[v], OrthoDir::West,
				infos[v].inList(OrthoDir::West).size() - 1 - flipedges)] = BendType::Bend1Left;
	...
	for (newbf = 0; newbf < newbendfree; newbf++) {
		m_abends[outEntry(infos[v], OrthoDir::West,
				infos[v].inList(OrthoDir::West).size() - 1 - flip_br - newbf)] = BendType::BendFree;
		m_agp_x[outEntry(infos[v], OrthoDir::West,
				infos[v].inList(OrthoDir::West).size() - 1 - flip_br - newbf)] =
				cp_x(outEntry(infos[v], OrthoDir::West,
						infos[v].inList(OrthoDir::West).size() - 1 - flip_br - newbf));
	}
...
void EdgeRouter::initialize_node_info(node v, int sep) {
	...
	{ //construct the edge lists for the incoming edges on ev side
		...
		if (adj != nullptr) {
			...
			do {
				...
				while (m_orp->direction(sadj)
						== m_orp->direction(adjSucc)) //edges may never be attached at corners
				{
					...
					if (infos[v].firstAdj() == nullptr) {
						infos[v].firstAdj() = in_edge_adj;
					}
					...
					if (is_in) {
						...
						//align test
						if (m_prup->typeOf(in_edge->source())
						...
						//align test
						if (m_prup->typeOf(in_edge->target())
								== Graph::NodeType::generalizationExpander) {
							if (m_align) {
								m_mergerSon[v] = true;
							}
	...
	if (infos[v].has_gen(OrthoDir::North)) {
		...
		dval = min(dsep, sep);
		OGDF_ASSERT(dval > 0);
		infos[v].set_delta(OrthoDir(0), OrthoDir(3), dval);
		infos[v].set_eps(OrthoDir(0), OrthoDir(3), int(floor(Cconst * dval)));
		//top side
		dsep = (re + Cconst == 0) ? sep : int(floor(infos[v].node_ysize() / (2 * (re + Cconst))));
		dval = min(dsep, sep);
		OGDF_ASSERT(dval > 0);
		infos[v].set_delta(OrthoDir(0), OrthoDir(1), dval);
		infos[v].set_eps(OrthoDir(0), OrthoDir(1), int(floor(Cconst * dval)));
	} else {
		int ae = vinfo->m_side[0].m_nAttached[0];
		if (ae > 0) {
			dsep = (ae + Cconst == 1)
					? sep
					: int(floor(infos[v].node_ysize() / (ae - 1 + 2 * Cconst))); // may be < 0
		} else {
			dsep = sep;
		}
		dval = min(dsep, sep);
		OGDF_ASSERT(dval > 0);
		if (dval >= infos[v].node_ysize()) {
			// allow 1 flip
			dval = int(floor((double)(infos[v].node_ysize()) / 2));
		}
	...
	if (infos[v].has_gen(OrthoDir::East)) {
		...
		dval = min(dsep, sep);
		OGDF_ASSERT(dval > 0);
		infos[v].set_delta(OrthoDir::East, OrthoDir::North, dval);
		infos[v].set_eps(OrthoDir::East, OrthoDir::North, int(floor(Cconst * dval)));
		//top side
		dsep = (re + Cconst == 0)
				? sep
				: int(floor((double)(infos[v].node_xsize()) / (2 * (re + Cconst))));
		dval = min(dsep, sep);
		OGDF_ASSERT(dval > 0);
		infos[v].set_delta(OrthoDir::East, OrthoDir::South, dval);
		infos[v].set_eps(OrthoDir::East, OrthoDir::South, int(floor(Cconst * dval)));
	} else {
		int ae = vinfo->m_side[1].m_nAttached[0];
		if (ae > 0) {
			dsep = (ae + Cconst == 1)
					? sep
					: int(floor((double)(infos[v].node_xsize()) / (ae - 1 + 2 * Cconst))); // may be <= 0
		} else {
			dsep = sep < int(floor((double)(infos[v].node_xsize()) / 2))
					? sep
					: int(floor((double)(infos[v].node_xsize()) / 2));
		}
		dval = min(dsep, sep);
		OGDF_ASSERT(dval > 0);
	...
	if (infos[v].has_gen(OrthoDir::South)) {
		...
		dsep = (le + Cconst == 0)
				? sep
				: int(floor((double)(infos[v].node_ysize()) / (2 * (le + Cconst))));
		dval = min(dsep, sep);
		OGDF_ASSERT(dval > 0);
		infos[v].set_delta(OrthoDir::South, OrthoDir::East, dval);
		infos[v].set_eps(OrthoDir::South, OrthoDir::East, int(floor(Cconst * dval)));
		//top side
		dsep = (re + Cconst == 0)
				? sep
				: int(floor((double)(infos[v].node_ysize()) / (2 * (re + Cconst))));
		dval = min(dsep, sep);
		infos[v].set_delta(OrthoDir::South, OrthoDir::West, dval);
		infos[v].set_eps(OrthoDir::South, OrthoDir::West, int(floor(Cconst * dval)));
	} else {
	...
	if (infos[v].has_gen(OrthoDir::West)) {
		...
		dsep = (le + Cconst == 0)
				? sep
				: int(floor((double)(infos[v].node_xsize()) / (2 * (le + Cconst))));
		dval = min(dsep, sep);
		OGDF_ASSERT(dval > 0);
		infos[v].set_delta(OrthoDir::West, OrthoDir::South, dval);
		infos[v].set_eps(OrthoDir::West, OrthoDir::South, int(floor(Cconst * dval)));
		//top side
		dsep = (re + Cconst == 0)
				? sep
				: int(floor((double)(infos[v].node_xsize()) / (2 * (re + Cconst))));
		dval = min(dsep, sep);
		infos[v].set_delta(OrthoDir::West, OrthoDir::North, dval);
		infos[v].set_eps(OrthoDir::West, OrthoDir::North, int(floor(Cconst * dval)));
	} else {
...
int EdgeRouter::beta_move(OrthoDir s_from, OrthoDir s_to, int move_num, node v) {
	...
	{ //debug top side to
		...
		if ((m_abends[ae] == BendType::BendFree) || (m_abends[ae] == bt1) || (m_abends[ae] == bt2)
				|| (m_abends[ae] == bt3) || (m_abends[ae] == bt4)) {
			return 0;
		}
		...
		case OrthoDir::East: //from left and right
			bend_saveable = (cp_y(ae) <= (infos[v].coord(s_to) - infos[v].delta(s_from, s_to) * ic
									 - infos[v].eps(s_from, s_to)));
			in_E_sfrom_sto = (cp_y(ae) > gp_y(ae));
			break;
		case OrthoDir::North: //from top and bottom
			bend_saveable = (cp_x(ae) >= (infos[v].coord(s_to) + infos[v].delta(s_from, s_to) * ic
									 + infos[v].eps(s_from, s_to)));
		...
		while (ep.valid() && bend_saveable && in_E_sfrom_sto // models E_from_to set in paper
			...
			//four cases:
			if (ep.valid()) {
				if ((m_abends[ae] == BendType::BendFree) || (m_abends[ae] == bt1)
						|| (m_abends[ae] == bt2) || (m_abends[ae] == bt3) || (m_abends[ae] == bt4)) {
				...
				case OrthoDir::East: //from left and right , ersetzte cp(*ep) durch cp(ae)
					bend_saveable = (cp_y(ae)
							<= (infos[v].coord(OrthoDir::East) - infos[v].delta(s_from, s_to) * ic
									- infos[v].eps(s_from, s_to)));
					in_E_sfrom_sto = (cp_y(ae) > gp_y(ae));
					break;
				case OrthoDir::North: //from top and bottom
					bend_saveable = (cp_x(ae)
							>= (infos[v].coord(OrthoDir::North) + infos[v].delta(s_from, s_to) * ic
									+ infos[v].eps(s_from, s_to)));
					in_E_sfrom_sto = (cp_x(ae) < gp_x(ae));
					break;
				case OrthoDir::South:
					bend_saveable = (cp_x(ae)
							<= (infos[v].coord(OrthoDir::South) - infos[v].delta(s_from, s_to) * ic
									- infos[v].eps(s_from, s_to)));
					in_E_sfrom_sto = (cp_x(ae) > gp_x(ae));
					break;
				case OrthoDir::West:
					bend_saveable = (cp_y(ae)
							>= (infos[v].coord(OrthoDir::West) + infos[v].delta(s_from, s_to) * ic
									+ infos[v].eps(s_from, s_to)));
					in_E_sfrom_sto = (cp_y(ae) < gp_y(ae));
					break;
				default:
...
int EdgeRouter::alpha_move(OrthoDir s_to, OrthoDir s_from, node v) {
	...
	if (infos[v].num_bend_free(s_to) != 0) {
		...
		result -= infos[v].delta(s_to, from)
				* (infos[v].num_bend_edges(s_to, from)
						+ infos[v].num_bend_edges(s_to, OrthoRep::oppDir(from)) - 1);
...
void EdgeRouter::setDistances() {
	for (node v : m_prup->nodes) {
		if ((m_prup->expandAdj(v) != nullptr)
				&& (m_prup->typeOf(v) != Graph::NodeType::generalizationMerger)) {
			OrthoDir od = OrthoDir::North;
			do {
...
void EdgeRouter::updateBends(const node v, ListIterator<edge>& it, int& pos, int& lastunbend,
...
int EdgeRouter::updateBends(const node v, ListIterator<edge>& it, const bool updateX,
		const OrthoDir dir, const bool bendLeft, const bool bendUp, int pos) {
	int lastunbend = m_init;
	updateBends(v, it, pos, lastunbend, updateX, dir, bendLeft, bendUp, false);
	return pos;
}

void EdgeRouter::updateLowerEdgesBends(const node v, ListIterator<edge>& it, int& pos, int& base,
	...
	while (it.valid()) // still some lower edges to bend
	{
		...
		updateOneBend(acp[adj] >= infos[v].coord(dir) - m_sep, adj, v, dir, bendLeft, btSingle,
				btDouble);
