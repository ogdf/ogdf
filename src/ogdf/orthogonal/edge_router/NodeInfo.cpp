...
#include <ogdf/basic/Graph_d.h>                    // for AdjElement, NodeArray, RegisteredArray...
#include <ogdf/basic/GridLayout.h>                 // for GridLayout
#include <ogdf/orthogonal/OrthoRep.h>              // for OrthoDir, OrthoRep
#include <ogdf/orthogonal/edge_router/NodeInfo.h>  // for NodeInfo, operator<<, RoutingChannel
...
//ACHTUNG: North ist 0, soll aber links sein
{
	...
	do {
		...
		if (sinfo.m_adjGen) {
			if ((od == OrthoDir::North) || (od == OrthoDir::East)) {
				set_gen_pos(od, sinfo.m_nAttached[0]);
			} else {
				set_gen_pos(od, sinfo.m_nAttached[1]);
			}
	...
	e = *ae; //pointing towards north, on left side
...
std::ostream& operator<<(std::ostream& O, const NodeInfo& inf) {
	...
	O.setf(ios::fixed);
#endif
	O << "box left/top/right/bottom: " << inf.coord(OrthoDir(0)) << "/" << inf.coord(OrthoDir(1))
	  << "/" << inf.coord(OrthoDir(2)) << "/" << inf.coord(OrthoDir(3)) << "\n"
	  << "boxsize:                   " << inf.box_x_size << ":" << inf.box_y_size << "\n"
	  << "cage l/t/r/b:              " << inf.cageCoord(OrthoDir(0)) << "/"
	  << inf.cageCoord(OrthoDir(1)) << "/" << inf.cageCoord(OrthoDir(2)) << "/"
	  << inf.cageCoord(OrthoDir(3)) << "\n"
	  << "gen. pos.:                 " << inf.gen_pos(OrthoDir(0)) << "/" << inf.gen_pos(OrthoDir(1))
	  << "/" << inf.gen_pos(OrthoDir(2)) << "/" << inf.gen_pos(OrthoDir(3)) << "\n"
	  << "delta l/t/r/b (left/right):" << inf.delta(OrthoDir::North, OrthoDir::West) << ":"
	  << inf.delta(OrthoDir::North, OrthoDir::East) << " / \n"
	  << "                          " << inf.delta(OrthoDir::East, OrthoDir::North) << ":"
	  << inf.delta(OrthoDir::East, OrthoDir::South) << " / \n"
	  << "                          " << inf.delta(OrthoDir::South, OrthoDir::East) << ":"
	  << inf.delta(OrthoDir::South, OrthoDir::West) << " / "
	  << inf.delta(OrthoDir::West, OrthoDir::South) << ":"
	  << inf.delta(OrthoDir::West, OrthoDir::North) << "\n"
	  << "eps l/t/r/b (left/right):  " << inf.eps(OrthoDir::North, OrthoDir::West) << ":"
	  << inf.eps(OrthoDir::North, OrthoDir::East) << " / \n"
	  << "                          " << inf.eps(OrthoDir::East, OrthoDir::North) << ":"
	  << inf.eps(OrthoDir::East, OrthoDir::South) << " / \n"
	  << "                          " << inf.eps(OrthoDir::South, OrthoDir::East) << ":"
	  << inf.eps(OrthoDir::South, OrthoDir::West) << " / "
	  << inf.eps(OrthoDir::West, OrthoDir::South) << ":" << inf.eps(OrthoDir::West, OrthoDir::North)
	  << "\n"
	  << "rc:                         " << inf.rc(OrthoDir(0)) << "/" << inf.rc(OrthoDir(1)) << "/"
	  << inf.rc(OrthoDir(2)) << "/" << inf.rc(OrthoDir(3)) << "\n"
	  << "num edges:                  " << inf.num_edges(OrthoDir(0)) << "/"
	  << inf.num_edges(OrthoDir(1)) << "/" << inf.num_edges(OrthoDir(2)) << "/"
	  << inf.num_edges(OrthoDir(3)) << "\n"
	  << "num bendfree edges:         " << inf.num_bend_free(OrthoDir(0)) << "/"
	  << inf.num_bend_free(OrthoDir(1)) << "/" << inf.num_bend_free(OrthoDir(2)) << "/"
