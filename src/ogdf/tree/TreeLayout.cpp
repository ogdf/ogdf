		...
		for (node v : tree.nodes) {
			...
			if (isLeaf(v)) {
				...
				while (first->cyclicSucc() != stop) {
					...
					m_leftSibling[first->theEdge()->target()] = previous->theEdge()->target();
					previous = first;
				}
...
void TreeLayout::callSortByPositions(GraphAttributes& AG, Graph& tree) {
	...
	for (node v : tree.nodes) {
		...
		for (adjEntry adj : v->adjEntries) {
			...
			if (m_orientation == Orientation::leftToRight
					|| m_orientation == Orientation::rightToLeft) {
				std::swap(dx, dy);
			}
			if (m_orientation == Orientation::topToBottom
					|| m_orientation == Orientation::rightToLeft) {
				dy = -dy;
			}
...
void TreeLayout::firstWalk(TreeStructure& ts, node subtree, bool upDown) {
	...
	if (ts.isLeaf(subtree)) {
		...
		double midpoint =
				(ts.m_preliminary[children.front()] + ts.m_preliminary[children.back()]) / 2;
		node leftSibling = ts.m_leftSibling[subtree];
		if (leftSibling != nullptr) {
			if (upDown) {
				ts.m_preliminary[subtree] = ts.m_preliminary[leftSibling]
			...
			ts.m_modifier[subtree] = ts.m_preliminary[subtree] - midpoint;
		} else {
...
void TreeLayout::apportion(TreeStructure& ts, node subtree, node& defaultAncestor, bool upDown) {
	OGDF_ASSERT(subtree != nullptr);
	...
	if (ts.m_leftSibling[subtree] == nullptr) {
		return;
	}
	...
	do {
		...
		if (ts.nextOnLeftContour(leftContourOut) != nullptr
				&& ts.nextOnRightContour(rightContourOut) != nullptr) {
			// continue traversal
			...
			if (upDown) {
				moveDistance = ts.m_preliminary[leftContourIn] + leftModSumIn
						...
						+ m_subtreeDistance - ts.m_preliminary[rightContourIn] - rightModSumIn;
			} else {
				moveDistance = ts.m_preliminary[leftContourIn] + leftModSumIn
			...
			if (moveDistance > 0) {
				...
				if (ts.m_parent[ts.m_ancestor[leftContourIn]] == ts.m_parent[subtree]) {
					...
					leftAncestor = defaultAncestor;
				}
				rightAncestor = subtree;

				// compute the number of small subtrees in between (plus 1)
				numberOfSubtrees = ts.m_number[rightAncestor] - ts.m_number[leftAncestor];
	...
	// adjust threads
	if (ts.nextOnRightContour(rightContourOut) == nullptr
			&& ts.nextOnRightContour(leftContourIn) != nullptr) {
	...
	if (ts.nextOnLeftContour(leftContourOut) == nullptr
			&& ts.nextOnLeftContour(rightContourIn) != nullptr) {
...
void TreeLayout::secondWalkX(TreeStructure& ts, node subtree, double modifierSum) {
	OGDF_ASSERT(subtree != nullptr);
	OGDF_ASSERT(subtree->graphOf() == ts.m_preliminary.graphOf());
	OGDF_ASSERT(subtree->graphOf() == ts.m_modifier.graphOf());
...
void TreeLayout::secondWalkY(TreeStructure& ts, node subtree, double modifierSum) {
	OGDF_ASSERT(subtree != nullptr);
	OGDF_ASSERT(subtree->graphOf() == ts.m_preliminary.graphOf());
	OGDF_ASSERT(subtree->graphOf() == ts.m_modifier.graphOf());
			...
			for (adjEntry adj : v->adjEntries) {
				...
				if (e->target() != v) {
					...
					if (m_orthogonalLayout) {
						edgeCoordinate = yCoordinate + (oldHeight + m_levelDistance) / 2;
		...
		while (!oldLevel.empty()) {
			...
			for (adjEntry adj : v->adjEntries) {
				...
				if (e->target() != v) {
					...
					edgeBends.clear();
					if (m_orthogonalLayout) {
						edgeCoordinate = xCoordinate + (oldWidth + m_levelDistance) / 2;
