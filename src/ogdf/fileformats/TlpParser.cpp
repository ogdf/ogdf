...
inline void Parser::tokenError(const char* str, bool got) {
	...
	if (m_begin == m_end) {
		...
		if (got) {
			GraphIO::logger.lout() << str << " at " << m_begin->line << ", " << m_begin->column
								   << " (got " << *m_begin << ")." << std::endl;
		} else {
			GraphIO::logger.lout() << str << " at " << m_begin->line << ", " << m_begin->column
								   << "." << std::endl;
		}
...
inline void Parser::tokenError(const std::string& str, bool got) { tokenError(str.c_str(), got); }
...
bool Parser::readEdge(Graph& G) {
	...
	int id, sid, tid;
	if (!(ss >> id >> sid >> tid)) {
		tokenError("incorrect edge statement format "
				   "(got \""
						+ ss.str() + "\", expected (\"int int int\")",
				false);
		return false;
	}
	...
	if (!source || !target) {
		GraphIO::logger.lout()
				<< "Node with id " << sid << " or " << tid << " is not declared." << std::endl;
		return false;
	}
...
inline bool Parser::applyNodes(Graph& G, ClusterGraph* C, cluster c, const std::string& str) {
	/*
...
static inline bool setAttribute(GraphAttributes& GA, node v, const Attribute& attr,
	...
	case Attribute::strokeColor:
		if (attrs & GraphAttributes::nodeStyle) {
			...
			int r, g, b, a;
			is >> TokenIgnorer('(') >> r >> TokenIgnorer(',') >> g >> TokenIgnorer(',') >> b
					>> TokenIgnorer(',') >> a >> TokenIgnorer(')');
	...
	case Attribute::position:
		if (attrs & GraphAttributes::nodeGraphics) {
			std::istringstream is(value);
			double x, y, z;
			is >> TokenIgnorer('(') >> x >> TokenIgnorer(',') >> y >> TokenIgnorer(',') >> z
					>> TokenIgnorer(')');
			GA.x(v) = x;
			GA.y(v) = y;
...
static inline bool setAttribute(GraphAttributes& GA, edge e, const Attribute& attr,
		const std::string& value) {
	const long attrs = GA.attributes();
...
bool Parser::readProperty(Graph& G, GraphAttributes* GA) {
	...
	while (m_begin != m_end && m_begin->leftParen()) {
		++m_begin;
		if (!readPropertyStatement(GA, attr, doneNode, nodeDefault, doneEdge, edgeDefault)) {
			return false;
		}
	}
...
bool Parser::readPropertyStatement(GraphAttributes* GA, const Attribute& attr,
		NodeArray<bool>& nodeDone, std::string& nodeDefault, EdgeArray<bool>& edgeDone,
...
bool Parser::readStatement(Graph& G, GraphAttributes* GA, ClusterGraph* C) {
	...
	if (head == "date") {
		...
		GraphIO::logger.lout(Logger::Level::Minor)
				<< "Unknown statement \"" << head << "\", ignoring.\n"
				<< std::endl;
