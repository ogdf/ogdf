...
#include <ogdf/basic/graphics.h>         // for Color, fromString, FillPattern (ptr only), Strok...
#include <ogdf/fileformats/GDF.h>        // for NodeAttribute, EdgeAttribute, toShape, toEdgeAtt...
...
#include <sstream>                       // for basic_ostream, basic_istringstream, basic_istream
#include <string>                        // for basic_string, char_traits, string, allocator
...
static inline bool readDef(const std::string& str, Attr toAttribute(const std::string& str),
		Attr a_unknown, std::vector<Attr>& attrs) {
	...
	while (std::getline(is, attr, ',')) {
		...
		if (attrib == a_unknown) {
			GraphIO::logger.lout(Logger::Level::Minor) << "attribute \"" << name << "\""
...
static size_t scanQuoted(const std::string& str, size_t pos, std::string& buff) {
...
static bool split(const std::string& str, std::vector<std::string>& result) {
	result.clear();
	std::string buff = "";
...
bool Parser::readNodeStmt(Graph& G, GraphAttributes* GA, const std::string& str, size_t line) {
...
bool Parser::readEdgeStmt(Graph& G, GraphAttributes* GA, const std::string& str, size_t line) {
	std::vector<std::string> values;
	split(str, values);
...
static bool inline readAttribute(GraphAttributes& GA, node v, const NodeAttribute& attr,
...
static bool inline readAttribute(GraphAttributes& GA, edge e, const EdgeAttribute& attr,
		const std::string& value) {
	const long attrs = GA.attributes();
...
static inline bool readAttrs(GraphAttributes& GA, T elem, const std::vector<A>& attrs,
...
bool Parser::readAttributes(GraphAttributes& GA, node v, const std::vector<std::string>& values) {
	return readAttrs(GA, v, m_nodeAttrs, values);
}

bool Parser::readAttributes(GraphAttributes& GA, edge e, const std::vector<std::string>& values) {
...
bool gdf::Parser::readGraph(Graph& G, GraphAttributes* GA) {
	G.clear();
