...
struct PairNodeItem {
	...
	explicit PairNodeItem(node v, ListIterator<PairFaceItem> it = ListIterator<PairFaceItem>())
		: m_v(v), m_it(it) { }
...
public:
	...
	// constructor
	ComputeBicOrder(const Graph& G, // biconnected planar graph
	...
	bool isPossNode(node v) { // precond.: v on C_k'
		return !m_onBase[v] && m_cutf[v] <= 1 && m_cutf[v] == virte(v) && m_numsf[v] == 0
				&& m_deg[v] >= 3;
	}
	...
	bool isPossVirt(node v) { // precond.: v on C_k'
		return m_virtEdge[v]
				&& ((m_deg[v] == 2 && v != m_vLeft) || (m_deg[next(v)] == 2 && next(v) != m_vRight));
	}
...
private:
	...
	int getBaseChain(ConstCombinatorialEmbedding& E, face f, double baseRatio, adjEntry& adjLeft,
			adjEntry& adjRight);

	adjEntry findMaxBaseChain(ConstCombinatorialEmbedding& E, face f, int& length);
	...
	// node v denotes virtual edge e = (v,next[v]), that satisfies (deg(v) = 2
	// and v != vLeft) or (deg(next(v)) = 2 and next(v) != vRight)
	ListPure<node> m_possVirt;
...
void ComputeBicOrder::print() {
	...
	for (node v = m_vLeft; v != nullptr; v = m_next[v]) {
		std::cout << " " << v << "[" << m_prev[v] << "," << m_prevPred[v] << " : " << m_next[v]
				  << "," << m_nextSucc[v] << "; " << m_virtEdge[v] << "]\n";
	}
	...
	for (face f : m_pEmbedding->faces) {
		std::cout << f->index() << ": outv = " << m_outv[f] << ", oute = " << m_oute[f]
				  << ", seqp = " << m_seqp[f] << ", isSF = " << m_isSf[f]
				  << ", virtSrc = " << m_virtSrc[f] << std::endl;
	}
	std::cout << std::endl;
}
...
#ifdef OGDF_BSO_OUTPUT
	std::cout << "vLeft = " << m_vLeft << ", "
			  << "vRight = " << m_vRight << std::endl;
#endif
...
void ComputeBicOrder::setV1(ShellingOrderSet& V) {
	...
	int i;
	adjEntry adj;
	for (i = 1, adj = m_adjLeft; i <= m_baseLength; i++, adj = adj->faceCycleSucc()) {
		V[i] = adj->theNode();
	}
}
...
void ComputeBicOrder::virtToContour(node v, node w, adjEntry adjNextSucc, adjEntry adjPrevPred) {
	m_next[v] = w;
	m_prev[w] = v;
...
void ComputeBicOrder::removeNextNode(ShellingOrderSet& V) {
	...
	for (node w : L_v) {
		...
		for (;;) {
			...
			if (v != w && m_onOuter[v]) {
				...
				virtToContour(adj2->theNode(), w, adj2,
						(w == next(m_nextV)) ? m_prevPred[w] : adj->twin()->cyclicPred());
	...
	if ((vVirt != nullptr && m_virtSrc[fVirt] == nullptr) || (vVirt == cl && m_virtSrc[fVirt] != cl)) {
...
int ComputeBicOrder::getBaseChain(ConstCombinatorialEmbedding& E, face f, double baseRatio,
		adjEntry& adjLeft, adjEntry& adjRight) {
	int len;
...
adjEntry ComputeBicOrder::findMaxBaseChain(ConstCombinatorialEmbedding& E, face f, int& length) {
	...
	for (adjEntry adj : f->entries) {
		i = num[adj->theNode()];
		adjEntry adj2;
		for (adj2 = adj->cyclicPred(); adj2 != adj->cyclicSucc(); adj2 = adj2->cyclicPred()) {
	...
	do {
		if (posInQ[i].valid()) {
			...
			if (d > length
					|| (d == length && adj2->theNode()->index() < adjStart->theNode()->index())) {
				length = d;
				adjStart = adj2;
			}
...
void BiconnectedShellingOrder::doCall(const Graph& G, adjEntry adj,
