...
node DavidsonHarel::computeCandidateLayout(const GraphAttributes& AG, DPoint& newPos) const {
...
#ifdef OGDF_DEBUG
	double dist = sqrt(
			(newPos.m_x - oldx) * (newPos.m_x - oldx) + (newPos.m_y - oldy) * (newPos.m_y - oldy));
...
void DavidsonHarel::placeIsolatedNodes(GraphAttributes& AG) const {
	...
	if (!m_nonIsolatedNodes.empty()) {
		...
		for (node v : m_nonIsolatedNodes) {
			...
			if (xVal - halfWidth < minX) {
				minX = xVal - halfWidth;
			}
			if (xVal + halfWidth > maxX) {
				maxX = xVal + halfWidth;
			}
			if (yVal - halfHeight < minY) {
				minY = yVal - halfHeight;
			}
			if (yVal + halfHeight > maxY) {
				maxY = yVal + halfHeight;
			}
		}
	}
	...
	for (node v : G.nodes) {
		if (v->degree() == 0) {
			isolated.pushBack(v);
			if (AG.height(v) > maxHeight) {
				maxHeight = AG.height(v);
			}
			if (AG.width(v) > maxWidth) {
				maxWidth = AG.width(v);
			}
		}
	}
