...
inline void eval_direct_aligned_SSE(float* ptr_x1, float* ptr_y1, float* ptr_s1, float* ptr_fx1,
		float* ptr_fy1, size_t n1, float* ptr_x2, float* ptr_y2, float* ptr_s2, float* ptr_fx2,
		float* ptr_fy2, size_t n2) {
...
#	endif
				...
				fx_sum_shuffled =
						_mm_shuffle_ps(fx_sum_shuffled, fx_sum_shuffled, _MM_SHUFFLE(0, 3, 2, 1));
				fy_sum_shuffled =
						_mm_shuffle_ps(fy_sum_shuffled, fy_sum_shuffled, _MM_SHUFFLE(0, 3, 2, 1));
			}
...
inline void eval_direct_aligned_SSE(float* ptr_x, float* ptr_y, float* ptr_s, float* ptr_fx,
		float* ptr_fy) {
#	if 0
...
#	endif
	...
	fx_sum = _mm_add_ps(fx_sum, _mm_sub_ps(fx, _mm_shuffle_ps(fx, fx, _MM_SHUFFLE(2, 1, 0, 3))));
	// ( fx(a,b), fx(b,c), fx(c,d), fx(d,a) ) - ( fx(d,a), fx(a,b), fx(b,c), fx(c,d) )
	fy_sum = _mm_add_ps(fy_sum, _mm_sub_ps(fy, _mm_shuffle_ps(fy, fy, _MM_SHUFFLE(2, 1, 0, 3))));
	// ( fy(a,b), fy(b,c), fy(c,d), fy(d,a) ) - ( fy(d,a), fy(a,b), fy(b,c), fy(c,d) )
...
void eval_direct_fast(float* x, float* y, float* s, float* fx, float* fy, size_t n) {
	if (n > 8) {
		...
		// eval BxA
		eval_direct(x + numA, y + numA, s + numA, fx + numA, fy + numA, numB, x, y, s, fx, fy, numA);
		// eval AxC
		eval_direct(x, y, s, fx, fy, numA, x + numAB, y + numAB, s + numAB, fx + numAB, fy + numAB,
				numC);
		// eval BxB
#	if 0
...
#	endif
		...
		// eval BxC
		eval_direct(x + numA, y + numA, s + numA, fx + numA, fy + numA, numB, x + numAB, y + numAB,
				s + numAB, fx + numAB, fy + numAB, numC);
...
void eval_direct_fast(float* x1, float* y1, float* s1, float* fx1, float* fy1, size_t n1, float* x2,
	...
	if ((n1 > 8) && (n2 > 8)) {
		...
		// eval A1 x ALL2
		eval_direct(x1, y1, s1, fx1, fy1, numA1, x2, y2, s2, fx2, fy2, n2);

		// eval C1x ALL2
		eval_direct(x1, y1, s1, fx1, fy1, numC1, x2, y2, s2, fx2, fy2, n2);

		// eval B1x A2
		eval_direct(x1 + numA1, y1 + numA1, s1 + numA1, fx1 + numA1, fy1 + numA1, numB1, x2, y2, s2,
				fx2, fy2, numA2);
...
void fast_multipole_l2p(double* localCoeffiecients, uint32_t numCoeffiecients, double centerX,
	...
	// + (source << 1))); //m_x[source], y - m_y[source]);
