...
bool pc_tree::PCTree::intersect(PCTree& other, PCTreeNodeArray<PCNode*>& mapping) {
    ...
    bool possible = other.findNodeRestrictions(*this, mapping, blockNodes, subtreeNodes, leafPartner, isFront);
...
bool pc_tree::PCTree::findNodeRestrictions(PCTree& applyTo, PCTreeNodeArray<PCNode*>& mapping,
    ...
    applyTo.changeRoot(mapping[leaves.back()] == applyTo.rootNode ? mapping[leaves.back()]->child1
    ...
    for (PCNode* node : fullNodeOrder) {
        ...
        for (auto it = std::next(neighbors.begin()); it != neighbors.end(); ++it) {
            ...
            if (leafPartner[mapping[current]] != nullptr) consecutiveOriginal.push_back(leafPartner[mapping[current]]);
            ...
            if (node->getNodeType() == PCNodeType::CNode && consecutiveOther.size() >= 2) {
                ...
                if (leafPartner[mapping[n1]] != nullptr) pair.push_back(leafPartner[mapping[n1]]);
                pair.push_back(mapping[n2]);
                if (leafPartner[mapping[n2]] != nullptr) pair.push_back(leafPartner[mapping[n2]]);
    ...
    while (!queue.empty()) {
        ...
        for (int i = 0; i < degree; i++) {
            ...
            if (!blockNodes[current].empty()) {
                ...
                PCNode *neighbor2 = leafPartner[current] == nullptr ? node->getNextNeighbor(previous, current)
                                                                    : node->getNextNeighbor(current,
                                                                                            leafPartner[current]);
