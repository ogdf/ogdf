...
void PCTree::LoggingObserver::labelsAssigned(
        PCTree& tree, PCNode* firstPartial, PCNode* lastPartial, int partialCount) {
...
void PCTree::LoggingObserver::terminalPathFound(
        PCTree& tree, PCNode* apex, PCNode* apexTPPred2, int terminalPathLength) {
    log << "Apex is " << apex->getLabel() << " " << apex << ", TP length is "
        << terminalPathLength << ", first path is [";
    int length = 1;
    ...
    while (firstPartial != nullptr) {
        ...
        if (firstPartial == nullptr && apexCandidate == nullptr) {
            ...
            if (parent_tinfo.tpPred == nullptr) {
                ...
                apexTPPred2 = node;
                if (parent->nodeType == PCNodeType::CNode && parent->getLabelUnchecked() == NodeLabel::Empty) {
            ...
            if (parent->nodeType == PCNodeType::CNode && parent->getLabelUnchecked() == NodeLabel::Partial) {
...
PCNode* PCTree::createCentralNode() {
    ...
    if (apexCandidate->nodeType == PCNodeType::PNode) {
        ...
        if (!std::equal(actual_neighbors.begin(), actual_neighbors.end(), cn.begin(), cn.end())) {
            actual_neighbors.push_back(actual_neighbors.front());
            actual_neighbors.pop_front();
            OGDF_ASSERT(std::equal(
                    actual_neighbors.rbegin(), actual_neighbors.rend(), cn.begin(), cn.end()));
        }
#endif
...
//                }
            ...
            for (auto obs : observers)
                obs->whenCNodeMerged(
                        *this, tpNeigh, tpNeighSiblingsFlipped, fullNeigh, fullOuterChild);
...
size_t PCTree::findEndOfFullBlock(
        PCNode* node, PCNode* pred, PCNode* curr, PCNode*& fullEnd, PCNode*& emptyEnd) const {
