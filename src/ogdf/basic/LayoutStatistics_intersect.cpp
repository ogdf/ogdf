...
namespace ogdf {
	...
	public:
		SweepCmpInternal(const DPointHandle &p) : m_pSweep(p) { }
	...
	public:
		SweepCmp(SweepCmpInternal *pInternal) : m_pInternal(pInternal) { }
	...
	typedef SortedSequence<DPointHandle,SeqItemY,EventCmp>    XSequence;
	typedef SortedSequence<DSegmentHandle,SeqItemXY,SweepCmp> YSequence;


	struct SeqItemY {
		SeqItemY() : m_iterY(), m_origNode(nullptr)  { }
		SeqItemY(YSequence::iterator iterY) : m_iterY(iterY), m_origNode(nullptr) { }
		SeqItemY(node vOrig) : m_iterY(), m_origNode(vOrig) { }
