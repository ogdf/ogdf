...
#include <ogdf/basic/graph_generators/deterministic.h>  // for circulantGraph, completeBipartite...
#include <random>                                       // for minstd_rand, uniform_int_distribu...
...
void completeBipartiteGraph(Graph& G, int n, int m) { completeKPartiteGraph(G, {n, m}); }
...
void globeGraph(Graph& G, int meridians, int latitudes) {
	...
	for (int lat = 0; lat < latitudes; lat++) {
		...
		for (int cross = 0; cross < meridians; ++cross) {
			node no = G.newNode();
			equator.pushBack(no);
