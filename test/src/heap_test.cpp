...
#include <ogdf/internal/heap/BinaryHeap.h>
#include <ogdf/internal/heap/PairingHeap.h>
#include <ogdf/internal/heap/BinomialHeap.h>
#include <ogdf/internal/heap/FibonacciHeap.h>
#include <ogdf/internal/heap/RMHeap.h>
#include <ogdf/internal/heap/RadixHeap.h>
#include <ogdf/internal/heap/HotQueue.h>
...
void simpleScenarioTest(bool supportsMerge)
{
	describe("simple scenario test", [&]() {
		typedef typename H<int, std::less<int>>::Handle Handle;
...
{
	typedef typename H<T, C>::Handle Handle;
...
void priorityQueueWrapperTest(std::size_t n)
{
	...
	describe(desc.data(), [&]() {
		...
		it("behaves like std::priority_queue", [&]() {
			...
			while(!ogdfPQ.empty() && !stdPQ.empty()) {
				AssertThat(ogdfPQ.top(), Equals(stdPQ.top()));
				AssertThat(ogdfPQ.size(), Equals(stdPQ.size()));
		...
		it_skip("supports move-construction", [&]() {
			ogdfPQ = init;
			...
			for(int e : init) {
				auto it = std::find(std::begin(elems), std::end(elems), e);
				AssertThat(it, Is().Not().EqualTo(std::end(elems)));

				std::swap(*it, elems.back());
				elems.pop_back();
			}
		});
...
void hotQueueSimpleScenario(std::size_t levels)
{
	typedef HotQueue<std::string, int, H> Queue;
...
template<template<typename T, class C> class H>
void hotQueueSimpleTest(std::size_t levels)
{
// TODO hot queue test does not compile on windows
	...
	describe(desc.data(), [&]() {
		 ...
		 */
		hotQueueSimpleScenario<H>(levels);
	});
#endif
}
...
template<template<typename T, class C> class H>
void describeHeapBasic(bool supportsMerge) {
	simpleScenarioTest<H>(supportsMerge);
	destructorTest<H>();
...
template<template<typename T, class C> class H>
void describeHeap(const char *title, bool supportsMerge = true) {
	describe(title, [&](){
		describeHeapBasic<H>(supportsMerge);

		describe("Heap-on-Top queue", []() {
			hotQueueSimpleTest<H>(3);
			hotQueueSimpleTest<H>(5);
			hotQueueSimpleTest<H>(7);
			hotQueueSimpleTest<H>(11);
		});
	});
}
...
go_bandit([]() {
	describe("Heaps", [](){
		describeHeap<BinaryHeap>("Binary heap", false);
		describeHeap<PairingHeap>("Pairing heap");
		describe_skip("BinomialHeap.decrease() seems flawed!", [](){
			describeHeap<BinomialHeap>("Binomial heap");
		});
