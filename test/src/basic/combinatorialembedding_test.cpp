...
void testCombinatorialEmbedding(Graph &graph) {
	...
	describe("splitting faces", [&] {
		...
		it("removes the edge when joining two faces", [&] {
			int numberOfFaces = emb.numberOfFaces();
			edge e = graph.chooseEdge([&](edge e) { return !emb.isBridge(e); });

			face faceLeft = emb.leftFace(e->adjSource());
			face faceRight = emb.rightFace(e->adjSource());

			int sizeLeft = faceLeft->size();
			int sizeRight = faceRight->size();

			face jointFace = emb.joinFaces(e);

			AssertThat(jointFace->size(), Equals(sizeLeft + sizeRight - 2));
			AssertThat(emb.numberOfFaces(), Equals(numberOfFaces - 1));
		});
	});
}
