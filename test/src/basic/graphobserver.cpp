...
go_bandit([]() {
	describe("GraphObserver", []() {
		...
		it("observes Graph::clear", []() {
			...
			AssertThat(GO.m_events.size(), Equals(1 + 1 + 5 + 7 + 1));
		...
		it("observes Graph::operator=", []() {
			...
			AssertThat(GO.m_events.size(), Equals(1 + 5 + 7));
			AssertThat(GO.m_events.at(0).first, Equals(GOEvent::CLEAR));
			for (int i = 0; i < 5 + 7; i++) {
		...
		it("observes Graph::insert(Graph)", []() {
			...
			AssertThat(GO.m_events.size(), Equals(5 + 7));
			for (int i = 0; i < 5 + 7; i++) {
		...
		it("cleanly unregisters from its Graph", []() {
			...
			G1.newNode(); // a callback now would cause a memory error
		});
	});
});
