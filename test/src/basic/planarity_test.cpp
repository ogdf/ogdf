...
go_bandit([](){
	...
	describe("Planar Subgraphs", [](){
		describe("Boyer-Myrvold-Subgraph", [](){
			...
			for(int n = 5; n < 30; n++) {
				it(string("works on a K" + to_string(n)), [&](){
					Graph graph;
			...
			for(int n = 5; n < 30; n++) {
				int m = n*(n-1)/3;
				it(string("works on a random graph with " + to_string(n) + " nodes and " + to_string(m) + " edges"), [&](){
					Graph graph;
			...
			for(int i = 0; i < 2; i++) {
				...
				for_each_graph_it(("works on a" + tags + " formerly failing instance"), instances,
						[&](Graph &graph, const string &filename){
