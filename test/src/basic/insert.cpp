...
protected:
	...
	void postInsert(void* userData, int newNodes, int newEdges) override {
		...
		if (m_notifyObservers) {
			AssertThat(newNodes, Equals(m_nodeSet.size()));
			AssertThat(newEdges, Equals(m_edgeSet.size()));
		} else {
			AssertThat(m_nodeSet.size(), Equals(0));
			AssertThat(m_edgeSet.size(), Equals(0));
		}
	}
};
...
void checkRes(int n, int m) {
	...
	if (ILG->m_notifyObservers) {
		AssertThat(ILG->m_nodeSet.size(), Equals(n));
		AssertThat(ILG->m_edgeSet.size(), Equals(m));
	}
}
...
void checkSameGraph(const NF& nodeFilter, const EF& edgeFilter, const NL& nodes, const EL& edges,
	...
	AssertThat(ILG->numberOfNodes(), Equals(nodes.size()));
	AssertThat(ILG->numberOfEdges(), Equals(edges.size()));
	if (notifyObservers) {
...
#define S(b) std::string(b ? "true" : "false")
	...
	describe("inserting the whole graph", [&]() {
		...
		it("works via insert(CCsInfo, ccNr)", [&] {
			...
			for (int i = 0; i < ccs.numberOfCCs(); ++i) {
				...
				AssertThat(ILG->m_nodeSet.size(), Equals(ccs.numberOfNodes(i)));
				AssertThat(ILG->m_edgeSet.size(), Equals(ccs.numberOfEdges(i)));
