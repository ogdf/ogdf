...
void describeMaxFlowModule(const string &name, const MaxFlowRequirement reqs = MFR_NONE)
{
	describe(name.c_str(), [&](){
		...
		for(int i = 0; i < MAX_FLOW_INSTANCES_COUNT; i++) {
			Graph graph;
			MaxFlowRequirement props;
			EdgeArray<VALUE_TYPE> caps;
			string instanceName;
			node s, t;
			VALUE_TYPE flow;
			getMaxFlowInstance<VALUE_TYPE>(i, instanceName, graph, caps, props, s, t, flow);

			// determine whether instance can be solved by this algorithm
			// meaning the graphs properties fulfill all requirements
			if((reqs & props) == reqs)  {
				// actually test the algorithm on this instance
				it(string("works on " + instanceName).c_str(), [&](){
					MAX_FLOW_ALGO alg(graph);

					VALUE_TYPE algFlow = alg.computeValue(caps, s, t);
					AssertThat(algFlow, Equals(flow));

					EdgeArray<VALUE_TYPE> algFlows(graph);
					alg.computeFlowAfterValue(algFlows);
					validateFlow(graph, caps, s, t, algFlows, algFlow);
				});
			}
		}
